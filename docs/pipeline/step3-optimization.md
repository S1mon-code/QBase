...
V6.0 扩展了 BacktestConfig 至约 30 个参数。在最终优化和验证阶段，建议启用 industrial-grade 配置以获得更真实的回测结果：
...
**注意：** 粗调阶段可以使用默认配置（更快），精调和最终验证时启用 industrial-grade 配置。详见 [AlphaForge API 参考](../reference/alphaforge-api.md#backtestconfigv60-扩展至-30-参数)。
