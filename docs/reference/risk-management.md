...
## Industrial 模式作为风险管理工具

Industrial 模式不仅是更准确的回测——它本身就是一个强大的风险管理工具。策略在 Industrial 模式下的 Sharpe 衰减程度，直接反映了该策略 alpha 对成本假设的依赖程度。

### 各频率预期 Sharpe 衰减范围

| 频率 | 预期衰减 | 风险等级 | 管理动作 |
|------|:-------:|:-------:|---------|
| daily | 5-10% | 低 | 正常使用，衰减可接受 |
| 4h | 10-25% | 中 | 监控衰减比，定期重验证 |
| 1h | 25-55% | 高 | 必须在 Industrial 下重新优化 |
| 5min | > 60% 可能 | 极高 | 仅适用 A 级高流动性品种 |

### 衰减的含义

- **衰减 < 10%** — 策略 alpha 来自真实市场信号，对成本不敏感
- **衰减 10-30%** — alpha 部分依赖于成本假设，但核心 alpha 仍在
- **衰减 > 30%** — 策略的 alpha 很大部分来自不真实的成本假设（如假设低量时也能按正常价差成交）
- **衰减 > 50%** — 策略不适合实盘，alpha 基本是虚假的
...
v9 的案例说明：1h 策略在 Basic 模式下有 261 笔交易，Industrial 模式下只剩 43 笔——大量"成交"在真实条件下根本不会发生。

### 风险管理规则

1. **Portfolio 中所有策略必须有 Industrial 模式验证记录**
2. Industrial 衰减 > 50% 的策略不入 Portfolio
3. 高频策略（1h+）的 Industrial 衰减纳入权重分配考量——衰减大的策略降低权重
4. 定期（每季度）用最新数据重跑 Industrial 验证，监控衰减趋势
