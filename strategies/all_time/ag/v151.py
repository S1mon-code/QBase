...
class StrategyV151(TimeSeriesStrategy):
    """
    策略简介：迁移学习(Ridge回归跨品种特征) + 相关性breakdown(跨市场regime) + Supertrend确认。

    使用指标：
    - Ridge Forecast(120): 用多特征Ridge回归预测方向（模拟迁移学习）
    - Correlation Breakdown: 跨市场regime检测，相关性稳定时信号更可靠
    - Supertrend(10, 3.0): 趋势方向确认
    - ATR(14): 止损距离计算

    进场条件（做多）：Ridge信号>0 + 相关性未breakdown + Supertrend方向=1
    进场条件（做空）：Ridge信号<0 + 相关性未breakdown + Supertrend方向=-1
    ...
    优点：Ridge正则化防过拟合，Supertrend双重确认
    缺点：Ridge需要特征矩阵，跨品种数据难获取
    """
    name = "ag_alltime_v151"
    warmup = 400
    freq = "4h"
    ...
    def __init__(self):
        ...
        self._ridge_signal = None
        self._ridge_conf = None
        self._st_dir = None
    ...
    def on_init_arrays(self, context, bars):
        ...
        # Build feature matrix from OHLCV-derived features
        ...
        self._ridge_signal, self._ridge_conf = rolling_ridge(
            closes, features, period=120
        )
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or atr_val <= 0:
            return
        ridge_sig = self._ridge_signal[i]
        st_dir = self._st_dir[i]
        if np.isnan(ridge_sig) or np.isnan(st_dir):
            return
