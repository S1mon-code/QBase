...
class SupertrendMonthlyStrategy(TimeSeriesStrategy):
    """
    策略简介：Supertrend趋势信号 + 月度季节性过滤的多空策略。

    使用指标：
    - Supertrend(10, 3.0): 趋势方向翻转信号
    - Monthly Seasonal(3年): 历史同月平均收益z-score
    - ATR(14): 止损距离计算
    ...
    出场条件：
    - ATR追踪止损 / 分层止盈 / Supertrend方向反转

    优点：季节性过滤掉逆季节信号，减少无效交易
    缺点：季节性效应可能不稳定，可能错过反季节大行情
    """
    name = "v103_supertrend_monthly"
    warmup = 300
    freq = "daily"

    st_period: int = 10             # Optuna: 7-20
    st_multiplier: float = 3.0     # Optuna: 2.0-5.0
    seasonal_threshold: float = 0.3 # Optuna: 0.1-0.5
    ...
    def on_bar(self, context):
        ...
        if np.isnan(seasonal):
            seasonal = 0.0  # neutral if no data
        ...
        # 3. Signal-based exit: supertrend flip
        ...
        # 4. Entry on supertrend flip with seasonal filter
        if side == 0:
            if prev_dir == -1 and st_dir == 1 and seasonal > -self.seasonal_threshold:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif prev_dir == 1 and st_dir == -1 and seasonal < self.seasonal_threshold:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
