...
class StrategyV183(TimeSeriesStrategy):
    """
    策略简介：Vote(Aroon osc>50/<-50, MACD hist sign, ROC sign) >= 2/3 + CMF.

    使用指标：
    - Aroon(25): oscillator direction vote
    - MACD(12,26,9): histogram direction vote
    - ROC(20): momentum direction vote
    ...
    优点：Diverse signals (trend + momentum + rate of change)
    缺点：Aroon and MACD may conflict in choppy transitions
    """
    name = "i_alltime_v183"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        roc_val = self._roc[i]
        cmf_val = self._cmf[i]
        if np.isnan(ao) or np.isnan(mh) or np.isnan(roc_val) or np.isnan(cmf_val):
            return
        ...
        if roc_val > 0: bull_votes += 1
        elif roc_val < 0: bear_votes += 1
