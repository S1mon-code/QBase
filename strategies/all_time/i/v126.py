...
class StrategyV126(TimeSeriesStrategy):
    """
    MTF: 4h PSAR direction + 1h PPO cross entry.
    Large TF: close > PSAR = bullish. Small TF: PPO line crosses signal.
    Strengths: PSAR clean, PPO normalized. Weaknesses: PSAR whipsaws.
    """
    name = "i_alltime_v126"
    freq = "1h"
    warmup = 800
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._psar_lg = psar(h_lg, l_lg, c_lg, self.psar_step, self.psar_max)
        self._close_lg = c_lg
    ...
    def on_bar(self, context):
        ...
        if i < 1:
            return
        pl_prev = self._ppo_line[i - 1]
        ps_prev = self._ppo_signal[i - 1]
        if np.isnan(pl_prev) or np.isnan(ps_prev):
            return
        j = self._lg_map[i]
        psar_val = self._psar_lg[j]
        cl = self._close_lg[j]
        if np.isnan(psar_val) or np.isnan(cl):
            return
        ...
        if side == 1 and not (cl > psar_val):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (cl < psar_val):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (cl > psar_val) and (pl_prev <= ps_prev and pl > ps):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < psar_val) and (pl_prev >= ps_prev and pl < ps):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
