...
class StrategyV74(TimeSeriesStrategy):
    """
    Keltner + CCI + Volume Momentum mean-reversion for Iron Ore (1h).

    Indicators:
    - Keltner Channel(20,1.5): ATR-based bands
    - CCI(20): commodity channel index for extreme detection
    - Volume Momentum(10): rate of change of volume

    Entry (Long): Price < lower KC + CCI < -120 + volume momentum > 0
    Entry (Short): Price > upper KC + CCI > +120 + volume momentum > 0
    Exit: Price returns to KC middle

    Pros: CCI + KC is a classic commodity mean-reversion setup
    Cons: CCI threshold needs careful tuning for iron ore volatility
    """
    name = "i_alltime_v74"
    warmup = 800
    freq = "1h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._vol_mom = volume_momentum(volumes, period=self.vm_period)
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(kc_u) or np.isnan(cci_val) or np.isnan(vm) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and c < kc_l and cci_val < -self.cci_threshold and vm > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and c > kc_u and cci_val > self.cci_threshold and vm > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, cci_val < -60):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, cci_val > 60):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
