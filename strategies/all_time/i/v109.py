...
class StrategyV109(TimeSeriesStrategy):
    """
    策略简介：NR7窄幅 + Aroon方向 + CMF资金流的1h突破策略。

    使用指标：NR7, Aroon(25), CMF(20), ATR(14)
    进场条件（做多）：近期NR7 + Aroon Up>Down + CMF>0
    进场条件（做空）：近期NR7 + Aroon Down>Up + CMF<0
    出场条件：ATR追踪止损 / 分层止盈 / Aroon反转
    优点：1h周期NR7信号更频繁
    缺点：1h NR7信号质量可能较低
    """
    name = "i_alltime_v109"
    ...
    def on_bar(self, context):
        ...
        for k in range(max(0, i - self.nr7_lookback), i + 1):
            if self._nr7[k]:
                recent_nr7 = True
                break
        aroon_bull = (ar_up - ar_down) > self.aroon_diff
        aroon_bear = (ar_down - ar_up) > self.aroon_diff
        ...
        # ── 3. 信号退出 ──
        if side == 1 and aroon_bear:
            context.close_long()
            self._reset_state()
        elif side == -1 and aroon_bull:
            context.close_short()
            self._reset_state()
        ...
        # ── 4. 入场 ──
        if side == 0:
            if recent_nr7 and aroon_bull and cmf_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif recent_nr7 and aroon_bear and cmf_val < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
