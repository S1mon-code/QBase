...
class StrategyV125(TimeSeriesStrategy):
    """
    MTF: 4h Ichimoku cloud direction + 1h RSI entry.
    Large TF: price above cloud = bullish. Small TF: RSI oversold pullback.
    Strengths: Ichimoku strong S/R. Weaknesses: cloud lags.
    """
    name = "i_alltime_v125"
    freq = "1h"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (cl > cloud_top) and (rsi_val < self.rsi_entry):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < cloud_bot) and (rsi_val > (100 - self.rsi_entry)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
