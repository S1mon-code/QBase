...
class StrategyV172(TimeSeriesStrategy):
    """
    策略简介：Kalman slope + MACD histogram + MFI confirmation on 4h.
    ...
    进场条件（做多）：Kalman slope > 0 且 MACD hist > 0 且 MFI < 80
    进场条件（做空）：Kalman slope < 0 且 MACD hist < 0 且 MFI > 20

    出场条件：ATR trailing stop / tiered profit / Kalman slope reversal

    优点：Triple confirmation reduces false entries
    缺点：May miss fast reversals due to Kalman smoothing
    """
    name = "i_alltime_v172"
    warmup = 500
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        mfi_val = self._mfi[i]
        if np.isnan(ks) or np.isnan(mh) or np.isnan(mfi_val):
            return
        ...
        # 4. Entry
        if side == 0:
            if ks > self.slope_thresh and mh > 0 and mfi_val < 80:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif ks < -self.slope_thresh and mh < 0 and mfi_val > 20:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
