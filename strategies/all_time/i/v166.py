...
class StrategyV166(TimeSeriesStrategy):
    """
    策略简介：HMM regime + RSI pullback entry + CMF flow confirmation on 4h.

    使用指标：
    - HMM Regime(120): market regime
    - RSI(14): oversold/overbought for entry timing
    - CMF(20): buying/selling pressure from money flow
    - ATR(14): stop distance

    进场条件（做多）：Regime=1 且 RSI < rsi_long 且 CMF > 0
    进场条件（做空）：Regime=2 且 RSI > rsi_short 且 CMF < 0

    出场条件：ATR trailing stop / tiered profit / regime flip

    优点：CMF adds volume-weighted confirmation to RSI signals
    缺点：4h regime detection needs more bars for stability
    """
    name = "i_alltime_v166"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        rsi_val = self._rsi[i]
        cmf_val = self._cmf[i]
        if np.isnan(rsi_val) or np.isnan(cmf_val):
            return
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and rsi_val < self.rsi_long and cmf_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and rsi_val > self.rsi_short and cmf_val < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
