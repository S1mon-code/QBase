...
class StrategyV15(TimeSeriesStrategy):
    """
    策略简介：Donchian通道突破 + ROC动量 + MFI量价确认的4h突破趋势策略。

    使用指标：
    - Donchian(20): 价格突破上轨做多，突破下轨做空
    - ROC(10): 变化率确认动量方向
    - MFI(14): 资金流量指标，确认量价一致

    进场条件（做多）：价格>=上轨 且 ROC>0 且 MFI>50
    进场条件（做空）：价格<=下轨 且 ROC<0 且 MFI<50

    出场条件：ATR追踪止损 / 分层止盈 / 价格回到中轨

    优点：Donchian突破是经典趋势入场法，MFI过滤假突破
    缺点：频繁假突破在震荡市中产生亏损
    """
    name = "i_alltime_v15"
    warmup = 400
    ...
    def on_bar(self, context):
        ...
        roc_val = self._roc[i]
        mfi_val = self._mfi[i]
        if np.isnan(don_u) or np.isnan(don_l) or np.isnan(roc_val) or np.isnan(mfi_val):
            return
        ...
        # 4. Entry
        if side == 0:
            if price >= don_u and roc_val > 0 and mfi_val > self.mfi_threshold:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price <= don_l and roc_val < 0 and mfi_val < (100 - self.mfi_threshold):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
