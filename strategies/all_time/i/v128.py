...
class StrategyV128(TimeSeriesStrategy):
    """
    MTF: 4h Donchian channel direction + 1h Williams %%R entry.
    Large TF: close > Donchian mid = bullish. Small TF: WR oversold.
    Strengths: Donchian robust, WR fast. Weaknesses: Donchian lags, WR volatile.
    """
    name = "i_alltime_v128"
    freq = "1h"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (cl > d_mid) and (wr_val < self.wr_entry):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < d_mid) and (wr_val > -(100 + self.wr_entry)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
