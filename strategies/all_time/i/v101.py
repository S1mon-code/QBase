...
class StrategyV101(TimeSeriesStrategy):
    """
    策略简介：TTM Squeeze释放 + Stochastic动量 + CMF资金流的4h突破策略。

    使用指标：
    - TTM Squeeze: 释放信号+momentum方向
    - Stochastic(14,3): K>D做多，K<D做空
    - CMF(20): 资金流向确认
    - ATR(14): 止损距离

    进场条件（做多）：Squeeze释放 + momentum>0 + K>D + CMF>0
    进场条件（做空）：Squeeze释放 + momentum<0 + K<D + CMF<0
    出场条件：ATR追踪止损 / 分层止盈 / Squeeze重新激活
    优点：Stochastic提供额外动量确认
    缺点：多条件叠加降低信号频率
    """
    name = "i_alltime_v101"
    warmup = 400
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._stoch_k, self._stoch_d = stochastic(highs, lows, closes, period=self.stoch_period)
    ...
    def on_bar(self, context):
        ...
        sd = self._stoch_d[i]
        cmf_val = self._cmf[i]
        if np.isnan(atr_val) or np.isnan(mom) or np.isnan(sk) or np.isnan(sd) or np.isnan(cmf_val):
            return
        ...
        # ── 4. 入场 ──
        if side == 0:
            if squeeze_release and mom > 0 and sk > sd and cmf_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif squeeze_release and mom < 0 and sk < sd and cmf_val < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
