...
class StrategyV46(TimeSeriesStrategy):
    """
    Bollinger Bands + Stochastic + OBV mean-reversion for Iron Ore (4h).

    Indicators:
    - Bollinger Bands(20, 2.0): volatility-based bands for extreme zones
    - Stochastic(14,3): %K/%D crossover in oversold/overbought zones
    - OBV: volume trend confirmation for accumulation/distribution

    Entry (Long):
    - Price below lower BB + Stochastic %K < 20 + OBV rising
    Entry (Short):
    - Price above upper BB + Stochastic %K > 80 + OBV falling
    Exit: Price returns to BB middle band

    Pros: 4h timeframe filters intraday noise while capturing swing reversals
    Cons: BB can expand in volatile markets, delaying mean-reversion signals
    """
    name = "i_alltime_v46"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        obv_val = self._obv[i]
        if np.isnan(bb_u) or np.isnan(sk) or np.isnan(atr_val) or np.isnan(obv_val):
            return
        ...
        # 4. Entry
        if side == 0 and c < bb_l and sk < self.stoch_oversold and obv_rising:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and c > bb_u and sk > self.stoch_overbought and obv_falling:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
