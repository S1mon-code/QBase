...
class StrategyV78(TimeSeriesStrategy):
    """
    Williams %R + Keltner + Volume Spike mean-reversion for Iron Ore (1h).

    Indicators:
    - Williams %R(14): bounded momentum oscillator
    - Keltner Channel(20,1.5): ATR-based bands
    - Volume Spike(20,2): abnormal volume detection

    Entry (Long): Williams %R < -85 + price < lower KC + volume spike
    Entry (Short): Williams %R > -15 + price > upper KC + volume spike
    Exit: Williams %R crosses -50

    Pros: Volume spikes on 1h are high-conviction reversal markers
    Cons: Volume spikes are rare; may have long periods without signals
    """
    name = "i_alltime_v78"
    warmup = 800
    freq = "1h"

    wr_oversold: float = -85.0
    wr_overbought: float = -15.0
    kc_period: int = 20
    kc_mult: float = 1.5
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(wr_val) or np.isnan(kc_u) or np.isnan(vs) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and wr_val < self.wr_oversold and c < kc_l and vs > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and wr_val > self.wr_overbought and c > kc_u and vs > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
