...
class StrategyV47(TimeSeriesStrategy):
    """
    RSI(14) mean reversion + ADX filter(<25=ranging) + MFI for Iron Ore (4h).

    Indicators:
    - RSI(14): oversold/overbought oscillator for reversal entries
    - ADX(14): trend strength filter; only trade when ADX < 25 (ranging market)
    - MFI(14): money flow index confirms volume-weighted pressure

    Entry (Long): RSI < rsi_oversold + ADX < adx_threshold + MFI < 30
    Entry (Short): RSI > rsi_overbought + ADX < adx_threshold + MFI > 70
    Exit: RSI crosses 50 (midpoint reversion)

    Pros: ADX filter ensures we only mean-revert in ranging markets
    Cons: May miss reversals at trend exhaustion points
    """
    name = "i_alltime_v47"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        rsi_val = self._rsi[i]
        adx_val = self._adx_val[i]
        mfi_val = self._mfi[i]
        atr_val = self._atr[i]
        if np.isnan(rsi_val) or np.isnan(adx_val) or np.isnan(mfi_val) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and rsi_val < self.rsi_oversold and adx_val < self.adx_threshold and mfi_val < self.mfi_threshold:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and rsi_val > self.rsi_overbought and adx_val < self.adx_threshold and mfi_val > (100 - self.mfi_threshold):
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
