...
class StrategyV157(TimeSeriesStrategy):
    """
    MTF: daily EMA ribbon + 4h PPO + Volume Momentum entry.
    Large TF: ribbon fast > slow. Small TF: PPO cross + vol momentum.
    Strengths: ribbon quality, PPO+vol dual. Weaknesses: ribbon noisy.
    """
    name = "i_alltime_v157"
    freq = "4h"
    warmup = 500
    ...
    def __init__(self):
        ...
        self._ribbon_fast_lg = None
        self._ribbon_slow_lg = None
        self._lg_map = None
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._ppo_line = pl
        self._ppo_signal = ps
        self._vol_mom = volume_momentum(volumes, 20)
        ...
        rf, rs, _ = ema_ribbon(c_lg, self.ribbon_fast, self.ribbon_slow)
        self._ribbon_fast_lg = rf
        self._ribbon_slow_lg = rs
    ...
    def on_bar(self, context):
        ...
        vm = self._vol_mom[i]
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(pl) or np.isnan(ps) or np.isnan(vm):
            return
        if i < 1:
            return
        ...
        if np.isnan(pl_p) or np.isnan(ps_p):
            return
        j = self._lg_map[i]
        rf = self._ribbon_fast_lg[j]
        rs = self._ribbon_slow_lg[j]
        if np.isnan(rf) or np.isnan(rs):
            return
        ...
        if side == 1 and not (rf > rs):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (rf < rs):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (rf > rs) and (pl_p <= ps_p and pl > ps and vm > 0):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (rf < rs) and (pl_p >= ps_p and pl < ps and vm < 0):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
