...
class StrategyV161(TimeSeriesStrategy):
    """
    策略简介：HMM regime detection + RSI oversold/overbought + ATR trailing stop.

    使用指标：
    - HMM Regime(120): 判断市场状态 (0=ranging, 1=bullish trend, 2=bearish trend)
    - RSI(14): 趋势中回调入场 / 区间边界反转
    - ATR(14): 止损距离计算

    进场条件（做多）：
    - Regime=1 (bullish) 且 RSI < rsi_entry_long (超卖回调)
    - Regime=0 (ranging) 且 RSI < rsi_revert_long (均值回归做多)

    进场条件（做空）：
    - Regime=2 (bearish) 且 RSI > rsi_entry_short (超买回调)
    - Regime=0 (ranging) 且 RSI > rsi_revert_short (均值回归做空)

    出场条件：
    - ATR 追踪止损 / 分层止盈（3ATR / 5ATR）
    - Regime 切换 → 平仓

    优点：根据市场状态自适应切换策略逻辑
    缺点：Regime 检测滞后，可能在转折点错过最佳入场
    """
    name = "i_alltime_v161"
    warmup = 400
    freq = "daily"
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and rsi_val < self.rsi_entry_long:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 0 and rsi_val < 30.0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
                    context.buy(base_lots)
                    self.entry_price = price
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and rsi_val > self.rsi_entry_short:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = -1
            elif regime == 0 and rsi_val > 70.0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
                    context.sell(base_lots)
                    self.entry_price = price
