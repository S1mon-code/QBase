...
class StrategyV68(TimeSeriesStrategy):
    """
    Detrended Price + Stochastic RSI + Force Index mean-reversion for Iron Ore (1h).

    Indicators:
    - DPO(20): cycle isolation, removes trend component
    - Stoch RSI(14,14,3,3): double-smoothed RSI for precise timing
    - Force Index(13): volume-weighted price momentum

    Entry (Long): DPO < -thresh + Stoch RSI %K < 0.2 + Force Index turning positive
    Entry (Short): DPO > +thresh + Stoch RSI %K > 0.8 + Force Index turning negative
    Exit: DPO crosses zero

    Pros: DPO isolates cycles; Stoch RSI is very responsive on 1h
    Cons: Triple condition may be too strict for 1h frequency
    """
    name = "i_alltime_v68"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(dpo_val) or np.isnan(srsi_k) or np.isnan(fi_val) or np.isnan(fi_prev) or np.isnan(atr_val):
            return
        ...
        if side == 0 and dpo_val < -self.dpo_threshold and srsi_k < self.srsi_oversold and fi_turn_pos:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and dpo_val > self.dpo_threshold and srsi_k > self.srsi_overbought and fi_turn_neg:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
