...
class StrategyV12(TimeSeriesStrategy):
    """
    策略简介：DEMA低延迟均线 + MACD柱确认 + OBV量价验证的4h趋势策略。

    使用指标：
    - DEMA(21): 双指数均线，延迟低于EMA，价格在其上方=多头
    - MACD(12,26,9): 柱状图确认动量方向
    - OBV: 累积量能方向确认

    进场条件（做多）：价格>DEMA 且 DEMA斜率>0 且 MACD柱>0 且 OBV上升
    进场条件（做空）：价格<DEMA 且 DEMA斜率<0 且 MACD柱<0 且 OBV下降

    出场条件：ATR追踪止损 / 分层止盈 / DEMA斜率反转+MACD确认

    优点：DEMA响应速度快，三指标交叉验证减少假信号
    缺点：DEMA在极端波动中可能过度反应
    """
    name = "i_alltime_v12"
    warmup = 400
    ...
    def on_bar(self, context):
        ...
        obv_val = self._obv[i]
        if np.isnan(dema_val) or np.isnan(hist_val) or np.isnan(obv_val):
            return
        dema_slope = 0.0
        ...
        # 3. Signal-based exit
        if side == 1 and (dema_slope < 0 and hist_val < 0):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and (dema_slope > 0 and hist_val > 0):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if price > dema_val and dema_slope > 0 and hist_val > 0 and obv_slope > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < dema_val and dema_slope < 0 and hist_val < 0 and obv_slope < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
