...
class StrategyV39(TimeSeriesStrategy):
    """
    策略简介：Fractal突破 + MACD动量 + Klinger量价振荡器的1h趋势策略。
    ...
    进场条件（做多）：突破分形高点 且 MACD柱>0 且 Klinger>信号
    进场条件（做空）：跌破分形低点 且 MACD柱<0 且 Klinger<信号

    出场条件：ATR追踪止损 / 分层止盈 / MACD+Klinger双反转

    优点：分形精确定位关键位，Klinger确认量价
    缺点：1h级别分形频繁，需要MACD严格过滤
    """
    name = "i_alltime_v39"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        if np.isnan(hist) or np.isnan(kl) or np.isnan(ks):
            return
        frac_up_break = False
        ...
        # 3. Signal-based exit
        if side == 1 and (hist < 0 and kl < ks):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and (hist > 0 and kl > ks):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if frac_up_break and hist > 0 and kl > ks:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif frac_down_break and hist < 0 and kl < ks:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
