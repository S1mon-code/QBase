...
class StrategyV55(TimeSeriesStrategy):
    """
    Price vs VWAP deviation + RSI + MFI mean-reversion for Iron Ore (4h).

    Indicators:
    - VWAP deviation: distance from VWAP as % of price; extreme = reversion opportunity
    - RSI(14): momentum confirmation
    - MFI(14): volume-weighted momentum

    Entry (Long): Price < VWAP * (1 - vwap_dev) + RSI < 35 + MFI < 30
    Entry (Short): Price > VWAP * (1 + vwap_dev) + RSI > 65 + MFI > 70
    Exit: Price returns to VWAP

    Pros: VWAP is the true volume-weighted fair value
    Cons: VWAP resets periodically and may not capture multi-day trends
    """
    name = "i_alltime_v55"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(vwap_val) or np.isnan(rsi_val) or np.isnan(mfi_val) or np.isnan(atr_val) or vwap_val < 1e-9:
            return
        ...
        # 3. Signal exit
        vwap_dist = (c - vwap_val) / vwap_val
        if side == 1 and c >= vwap_val:
            context.close_long()
            self._reset_state()
            return
        if side == -1 and c <= vwap_val:
        ...
        # 4. Entry
        if side == 0 and c < vwap_val * (1 - self.vwap_dev) and rsi_val < self.rsi_oversold and mfi_val < self.mfi_low:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and c > vwap_val * (1 + self.vwap_dev) and rsi_val > self.rsi_overbought and mfi_val > (100 - self.mfi_low):
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, rsi_val < 45):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, rsi_val > 55):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
