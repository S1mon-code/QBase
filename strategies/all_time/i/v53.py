...
from indicators.momentum.rvi import rvi
...
class StrategyV53(TimeSeriesStrategy):
    """
    RVI + Bollinger Bands + Force Index mean-reversion for Iron Ore (4h).

    Indicators:
    - RVI(10): Relative Vigor Index compares close-open vs high-low range
    - Bollinger Bands(20,2): statistical bands for extreme detection
    - Force Index(13): volume-weighted price change for exhaustion detection

    Entry (Long): RVI < -rvi_threshold + price < lower BB + force index turning positive
    Entry (Short): RVI > +rvi_threshold + price > upper BB + force index turning negative
    Exit: Price returns to BB middle

    Pros: RVI measures conviction of price moves; force index adds volume weight
    Cons: RVI can be noisy in choppy markets
    """
    name = "i_alltime_v53"
    warmup = 400
    freq = "4h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._rvi = rvi(opens, highs, lows, closes, period=self.rvi_period)
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(rvi_val) or np.isnan(bb_u) or np.isnan(fi_val) or np.isnan(atr_val) or np.isnan(fi_prev):
            return
        ...
        if side == 0 and rvi_val < -self.rvi_threshold and c < bb_l and fi_turn_pos:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and rvi_val > self.rvi_threshold and c > bb_u and fi_turn_neg:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, rvi_val < 0):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, rvi_val > 0):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
