...
class StrategyV179(TimeSeriesStrategy):
    """
    策略简介：Kalman slope + MACD histogram + OI Momentum on 1h.

    使用指标：
    - Kalman Filter: adaptive trend slope
    - MACD(12,26,9): momentum histogram
    - OI Momentum(20): open interest rate of change
    - ATR(14): stop distance

    进场条件（做多）：Kalman slope > 0 且 MACD hist > 0 且 OI rising
    进场条件（做空）：Kalman slope < 0 且 MACD hist < 0 且 OI rising

    出场条件：ATR trailing stop / tiered profit / Kalman slope reversal

    优点：Rising OI with trend confirms new money entering the trend
    缺点：OI data may not update at 1h granularity
    """
    name = "i_alltime_v179"
    warmup = 600
    freq = "1h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        _, _, self._macd_hist = macd(closes, self.macd_fast, 26, 9)
    ...
    def on_bar(self, context):
        ...
        if np.isnan(ks) or np.isnan(mh) or np.isnan(oi_val):
            return
        ...
        # 4. Entry
        if side == 0:
            if ks > self.slope_thresh and mh > 0 and oi_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif ks < -self.slope_thresh and mh < 0 and oi_val > 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
