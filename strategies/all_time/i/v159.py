...
class StrategyV159(TimeSeriesStrategy):
    """
    MTF: daily Supertrend+ADX + 4h RSI + OI Momentum entry.
    Large TF: ST bullish + ADX strong. Small TF: RSI pullback + OI mom > 0.
    Strengths: dual daily filter robust, OI unique. Weaknesses: restrictive, OI sparse.
    """
    name = "i_alltime_v159"
    freq = "4h"
    warmup = 500
    ...
    def on_init_arrays(self, context, bars):
        ...
        try:
            ...
            self._oi_mom = oi_momentum(oi_arr, 14)
        except Exception:
    ...
    def on_bar(self, context):
        ...
        rsi_val = self._rsi[i]
        oi_m = self._oi_mom[i]
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(rsi_val) or np.isnan(oi_m):
            return
        j = self._lg_map[i]
        st_dir = self._st_dir_lg[j]
        adx_v = self._adx_lg[j]
        if np.isnan(st_dir) or np.isnan(adx_v):
            return
        ...
        if side == 1 and not (st_dir == 1 and adx_v > self.adx_threshold):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (st_dir == -1 and adx_v > self.adx_threshold):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (st_dir == 1 and adx_v > self.adx_threshold) and (rsi_val < self.rsi_entry and oi_m > 0):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (st_dir == -1 and adx_v > self.adx_threshold) and (rsi_val > (100 - self.rsi_entry) and oi_m < 0):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
