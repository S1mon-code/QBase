...
class StrategyV167(TimeSeriesStrategy):
    """
    策略简介：HMM regime + MACD crossover + Klinger volume oscillator on 4h.

    使用指标：
    - HMM Regime(120): regime detection
    - MACD(12,26,9): trend momentum via histogram
    - Klinger(34,55,13): volume flow oscillator
    - ATR(14): stop distance

    进场条件（做多）：Regime=1 且 MACD hist > 0 且 Klinger > signal
    进场条件（做空）：Regime=2 且 MACD hist < 0 且 Klinger < signal

    出场条件：ATR trailing stop / tiered profit / regime flip

    优点：Klinger captures money flow direction with volume accumulation
    缺点：Both MACD and Klinger are lagging indicators
    """
    name = "i_alltime_v167"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        if np.isnan(macd_h) or np.isnan(kl) or np.isnan(ks):
            return
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and macd_h > 0 and kl > ks:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and macd_h < 0 and kl < ks:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
