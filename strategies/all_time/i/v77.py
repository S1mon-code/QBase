...
class StrategyV77(TimeSeriesStrategy):
    """
    CMO + Z-score + OBV mean-reversion for Iron Ore (1h).

    Indicators:
    - CMO(14): Chande Momentum Oscillator, bounded -100/+100
    - Z-score(20): statistical price deviation
    - OBV: cumulative volume trend

    Entry (Long): CMO < -50 + Z-score < -1.5 + OBV rising
    Entry (Short): CMO > +50 + Z-score > +1.5 + OBV falling
    Exit: CMO crosses zero

    Pros: CMO + Z-score gives dual statistical confirmation
    Cons: Both indicators may lag in fast intraday moves
    """
    name = "i_alltime_v77"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(cmo_val) or np.isnan(z) or np.isnan(obv_val) or np.isnan(obv_prev) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and cmo_val < -self.cmo_threshold and z < -self.zscore_threshold and obv_val > obv_prev:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and cmo_val > self.cmo_threshold and z > self.zscore_threshold and obv_val < obv_prev:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
