...
class StrategyV61(TimeSeriesStrategy):
    """
    Bollinger Bands + RSI + OBV mean-reversion for Iron Ore (1h).

    Indicators:
    - Bollinger Bands(20,2): statistical bands for extreme detection on 1h
    - RSI(14): momentum oscillator for oversold/overbought
    - OBV: volume trend for accumulation/distribution confirmation

    Entry (Long): Price < lower BB + RSI < 25 + OBV rising (10-bar)
    Entry (Short): Price > upper BB + RSI > 75 + OBV falling (10-bar)
    Exit: Price returns to BB middle band

    Pros: 1h gives more precise entries than daily; classic combo
    Cons: More noise on 1h; OBV can be misleading in thin markets
    """
    name = "i_alltime_v61"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(bb_u) or np.isnan(rsi_val) or np.isnan(obv_val) or np.isnan(obv_prev) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and c < bb_l and rsi_val < self.rsi_oversold and obv_val > obv_prev:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and c > bb_u and rsi_val > self.rsi_overbought and obv_val < obv_prev:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
