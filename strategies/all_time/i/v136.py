...
class StrategyV136(TimeSeriesStrategy):
    """
    MTF: 4h Supertrend + 1h MACD cross + CMF entry.
    Large TF: ST bullish. Small TF: MACD cross up + CMF > 0.
    Strengths: triple confirmation. Weaknesses: too many filters.
    """
    name = "i_alltime_v136"
    freq = "1h"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (st_dir == 1) and (ml_p <= ms_p and ml > ms and cmf_val > 0):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (st_dir == -1) and (ml_p >= ms_p and ml < ms and cmf_val < 0):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
