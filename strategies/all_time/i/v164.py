...
class StrategyV164(TimeSeriesStrategy):
    """
    策略简介：HMM regime + CCI extreme levels + Force Index direction.

    使用指标：
    - HMM Regime(120): market regime detection
    - CCI(20): commodity channel index for overbought/oversold
    - Force Index(13): volume-price force direction
    - ATR(14): stop distance

    进场条件（做多）：Regime=1 且 CCI < -cci_threshold 且 Force Index > 0
    进场条件（做空）：Regime=2 且 CCI > cci_threshold 且 Force Index < 0

    出场条件：ATR trailing stop / tiered profit / regime change

    优点：CCI captures cyclical extremes well in commodities
    缺点：Force Index can be erratic with volume spikes
    """
    name = "i_alltime_v164"
    warmup = 400
    freq = "daily"
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and cci_val < -self.cci_threshold and fi_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and cci_val > self.cci_threshold and fi_val < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
