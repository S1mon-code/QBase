...
class StrategyV120(TimeSeriesStrategy):
    """
    策略简介：Range Expansion + RSI动量 + OI Momentum的1h突破策略。

    使用指标：Range Expansion, RSI(14), OI Momentum, ATR(14)
    进场条件（做多）：RE触发 + RSI>50 + OI Mom>0
    进场条件（做空）：RE触发 + RSI<50 + OI Mom>0
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or np.isnan(rsi_val):
            return
        if np.isnan(oi_val):
            oi_val = 0.0
        ...
        # ── 4. 入场 ──
        if side == 0:
            if re_val and rsi_val > self.rsi_threshold and oi_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif re_val and rsi_val < self.rsi_threshold and oi_val > 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
