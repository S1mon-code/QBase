...
class StrategyV70(TimeSeriesStrategy):
    """
    Fisher Transform + Keltner + Volume Spike mean-reversion for Iron Ore (1h).

    Indicators:
    - Fisher Transform(10): Gaussian-normalized price extremes
    - Keltner Channel(20,1.5): ATR-based bands
    - Volume Spike(20,2): capitulation/climax volume

    Entry (Long): Fisher < -1.5 + price < lower KC + volume spike
    Entry (Short): Fisher > +1.5 + price > upper KC + volume spike
    Exit: Fisher crosses zero

    Pros: Fisher + volume spike is a powerful reversal combo on 1h
    Cons: Fisher can overshoot in strong trends
    """
    name = "i_alltime_v70"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(fv) or np.isnan(kc_u) or np.isnan(vs) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and fv < -self.fisher_threshold and c < kc_l and vs > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and fv > self.fisher_threshold and c > kc_u and vs > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
