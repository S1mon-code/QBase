...
class StrategyV40(TimeSeriesStrategy):
    """
    策略简介：EMA Ribbon趋势 + Stochastic动量 + 成交量动量的1h趋势策略。

    使用指标：
    - EMA Ribbon(8,21): 快慢EMA排列
    - Stochastic(14,3): K/D交叉
    - Volume Momentum(10): 量能动量

    进场条件（做多）：快EMA>慢EMA 且 K>D 且 K>50 且 量能>0
    进场条件（做空）：快EMA<慢EMA 且 K<D 且 K<50 且 量能>0

    出场条件：ATR追踪止损 / 分层止盈 / EMA Ribbon交叉

    优点：Ribbon宽度量化趋势，Stochastic+量能双确认
    缺点：1h级别EMA ribbon频繁交叉
    """
    name = "i_alltime_v40"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        sd = self._stoch_d[i]
        vm = self._vol_mom[i]
        if np.isnan(rf) or np.isnan(rs) or np.isnan(sk) or np.isnan(sd) or np.isnan(vm):
            return
        ...
        # 4. Entry
        if side == 0:
            if rf > rs and sk > sd and sk > self.stoch_threshold and vm > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif rf < rs and sk < sd and sk < (100 - self.stoch_threshold) and vm > 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
