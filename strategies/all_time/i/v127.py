...
class StrategyV127(TimeSeriesStrategy):
    """
    MTF: 4h Vortex direction + 1h ROC momentum entry.
    Large TF: +VI > -VI = bullish. Small TF: ROC > threshold.
    Strengths: Vortex captures strength, ROC confirms. Weaknesses: choppy in range.
    """
    name = "i_alltime_v127"
    freq = "1h"
    warmup = 800
    ...
    def __init__(self):
        ...
        self._plus_vi_lg = None
        self._minus_vi_lg = None
        self._lg_map = None
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._plus_vi_lg, self._minus_vi_lg = vortex(h_lg, l_lg, c_lg, self.vortex_period)
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(roc_val):
            return
        j = self._lg_map[i]
        pvi = self._plus_vi_lg[j]
        mvi = self._minus_vi_lg[j]
        if np.isnan(pvi) or np.isnan(mvi):
            return
        ...
        if side == 1 and not (pvi > mvi + self.vortex_diff):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (mvi > pvi + self.vortex_diff):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (pvi > mvi + self.vortex_diff) and (roc_val > self.roc_threshold):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (mvi > pvi + self.vortex_diff) and (roc_val < -self.roc_threshold):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
