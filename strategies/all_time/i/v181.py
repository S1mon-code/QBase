...
class StrategyV181(TimeSeriesStrategy):
    """
    策略简介：Vote(Supertrend dir, ADX>25, MACD hist>0) >= 2/3 + OBV confirmation.

    使用指标：
    - Supertrend(10,3): trend direction vote
    - ADX(14): trend strength vote (>25 = trending)
    - MACD(12,26,9): momentum direction vote (histogram sign)
    - OBV: volume flow confirmation filter
    - ATR(14): stop distance

    进场条件（做多）：>= 2/3 bullish votes 且 OBV rising
    进场条件（做空）：>= 2/3 bearish votes 且 OBV falling

    出场条件：ATR trailing stop / tiered profit / vote score drops to 0

    优点：Majority voting reduces single indicator noise
    缺点：All three are trend indicators, may not provide diverse signals
    """
    name = "i_alltime_v181"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        if np.isnan(st) or np.isnan(adx_v) or np.isnan(mh):
            return
