...
class StrategyV76(TimeSeriesStrategy):
    """
    Stochastic RSI + Bollinger + Force Index mean-reversion for Iron Ore (1h).

    Indicators:
    - Stoch RSI(14,14,3,3): double-smoothed RSI for precise reversal timing
    - Bollinger Bands(20,2): price envelope
    - Force Index(13): volume-weighted momentum

    Entry (Long): Stoch RSI %K < 0.15 + price < lower BB + force index turning positive
    Entry (Short): Stoch RSI %K > 0.85 + price > upper BB + force index turning negative
    Exit: Stoch RSI crosses 0.5

    Pros: Stoch RSI is more responsive than RSI; force index confirms volume
    Cons: Double-smoothing can cause lag in fast markets
    """
    name = "i_alltime_v76"
    warmup = 800
    freq = "1h"

    srsi_oversold: float = 0.15
    srsi_overbought: float = 0.85
    bb_period: int = 20
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._srsi_k, _ = stoch_rsi(closes, rsi_period=14, stoch_period=14, k_smooth=3, d_smooth=3)
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(srsi_k) or np.isnan(bb_u) or np.isnan(fi_val) or np.isnan(fi_prev) or np.isnan(atr_val):
            return
        ...
        if side == 0 and srsi_k < self.srsi_oversold and c < bb_l and fi_turn_pos:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and srsi_k > self.srsi_overbought and c > bb_u and fi_turn_neg:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, srsi_k < 0.3):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, srsi_k > 0.7):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
