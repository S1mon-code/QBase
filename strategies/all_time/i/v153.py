...
class StrategyV153(TimeSeriesStrategy):
    """
    MTF: daily Linear Reg slope + 4h CCI + MFI entry.
    Large TF: LR slope > 0. Small TF: CCI extreme + MFI confirm.
    Strengths: LR clean trend, CCI+MFI dual. Weaknesses: many filters, LR lags.
    """
    name = "i_alltime_v153"
    freq = "4h"
    warmup = 500
    ...
    def on_init_arrays(self, context, bars):
        ...
        _, slope, _ = linear_regression(c_lg, self.lr_period)
        self._lr_slope_lg = slope
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not (slope > 0):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (slope < 0):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (slope > 0) and (cci_val < -self.cci_threshold and mfi_val < self.mfi_threshold):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (slope < 0) and (cci_val > self.cci_threshold and mfi_val > (100 - self.mfi_threshold)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
