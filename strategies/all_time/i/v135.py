...
class StrategyV135(TimeSeriesStrategy):
    """
    MTF: 4h KAMA direction + 1h Stochastic + OBV entry.
    Large TF: KAMA rising = bullish. Small TF: Stoch oversold + OBV rising.
    Strengths: KAMA adaptive, OBV confirms. Weaknesses: OBV can diverge.
    """
    name = "i_alltime_v135"
    freq = "1h"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        k_prev = self._kama_lg[max(0, j - 2)]
        ...
        # 4. Entry
        if side == 0:
            if (k_now > k_prev) and (sk < self.stoch_entry and obv_rising):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (k_now < k_prev) and (sk > (100 - self.stoch_entry) and obv_falling):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
