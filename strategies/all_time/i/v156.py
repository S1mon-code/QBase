...
class StrategyV156(TimeSeriesStrategy):
    """
    MTF: daily Ichimoku cloud + 4h ROC + Klinger entry.
    Large TF: above cloud. Small TF: ROC > threshold + Klinger confirm.
    Strengths: comprehensive, Klinger vol-based. Weaknesses: many conditions.
    """
    name = "i_alltime_v156"
    freq = "4h"
    warmup = 500
    ...
    def on_init_arrays(self, context, bars):
        ...
        kl, ks = klinger(highs, lows, closes, volumes)
        self._klinger_line = kl
        self._klinger_signal = ks
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        roc_val = self._roc[i]
        kl_v = self._klinger_line[i]
        ks_v = self._klinger_signal[i]
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(roc_val) or np.isnan(kl_v) or np.isnan(ks_v):
            return
        j = self._lg_map[i]
        ...
        # 4. Entry
        if side == 0:
            if (cl > cloud_top) and (roc_val > self.roc_threshold and kl_v > ks_v):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < cloud_bot) and (roc_val < -self.roc_threshold and kl_v < ks_v):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
