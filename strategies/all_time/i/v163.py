...
class StrategyV163(TimeSeriesStrategy):
    """
    策略简介：HMM regime + Stochastic overbought/oversold + MFI confirmation.

    使用指标：
    - HMM Regime(120): market state detection
    - Stochastic(14,3): momentum oscillator for entry timing
    - MFI(14): money flow index for volume-price confirmation
    - ATR(14): stop distance

    进场条件（做多）：Regime=1 且 Stoch %K < stoch_oversold 且 MFI < 40
    进场条件（做空）：Regime=2 且 Stoch %K > stoch_overbought 且 MFI > 60

    出场条件：ATR trailing stop / tiered profit / regime change

    优点：Stochastic gives precise entry timing within trends
    缺点：In strong trends Stochastic stays overbought, limiting entries
    """
    name = "i_alltime_v163"
    warmup = 400
    freq = "daily"
    ...
    def on_bar(self, context):
        ...
        mfi_val = self._mfi[i]
        if np.isnan(stoch_val) or np.isnan(mfi_val):
            return
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and stoch_val < self.stoch_oversold and mfi_val < 40:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and stoch_val > self.stoch_overbought and mfi_val > 60:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
