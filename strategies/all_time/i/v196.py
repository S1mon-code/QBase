...
class StrategyV196(TimeSeriesStrategy):
    """
    策略简介：Dual-mode: ADX>25→trend(Supertrend), ADX<20→revert(RSI) + OBV on 1h.

    使用指标：
    - ADX(14): regime switch
    - Supertrend(10,3): trend-following in trend mode
    - RSI(14): oversold/overbought in range mode
    - OBV: volume flow confirmation
    - ATR(14): stop distance

    进场条件（趋势-做多）：ADX>25 且 ST=1 且 OBV rising
    进场条件（回归-做多）：ADX<20 且 RSI<30 且 OBV rising
    进场条件（趋势-做空）：ADX>25 且 ST=-1 且 OBV falling
    进场条件（回归-做空）：ADX<20 且 RSI>70 且 OBV falling

    出场条件：ATR trailing stop / tiered profit / mode exit

    优点：1h provides more granular entry timing
    缺点：ADX is noisier on 1h, regime detection less stable
    """
    name = "i_alltime_v196"
    warmup = 600
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        st = self._st_dir[i]
        rv = self._rsi[i]
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(adx_v) or np.isnan(st) or np.isnan(rv):
            return
        ...
        # 4. Entry
        if side == 0:
            if (is_trend and st == 1 and obv_up) or (is_range and rv < 30 and obv_up):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (is_trend and st == -1 and not obv_up) or (is_range and rv > 70 and not obv_up):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
