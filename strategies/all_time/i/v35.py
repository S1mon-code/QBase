...
class StrategyV35(TimeSeriesStrategy):
    """
    策略简介：TEMA三重均线 + PPO标准化动量 + 放量触发的1h趋势策略。

    使用指标：
    - TEMA(15): 三重指数均线
    - PPO(12,26,9): 百分比振荡器
    - Volume Spike(20,1.5): 放量检测

    进场条件（做多）：价格>TEMA 且 斜率>0 且 PPO>信号 且 放量
    进场条件（做空）：价格<TEMA 且 斜率<0 且 PPO<信号 且 放量

    出场条件：ATR追踪止损 / 分层止盈 / TEMA+PPO双反转

    优点：TEMA低延迟，放量确认突破有效性
    缺点：1h级别放量频繁，可能产生过多信号
    """
    name = "i_alltime_v35"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        if np.isnan(tema_val) or np.isnan(ppo_l) or np.isnan(ppo_s) or np.isnan(spike):
            return
        tema_slope = 0.0
        ...
        # 4. Entry
        if side == 0:
            if price > tema_val and tema_slope > 0 and ppo_l > ppo_s and spike > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < tema_val and tema_slope < 0 and ppo_l < ppo_s and spike > 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
