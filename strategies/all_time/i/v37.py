...
class StrategyV37(TimeSeriesStrategy):
    """
    策略简介：ZLEMA零延迟均线 + ADX趋势强度 + CMF资金流的1h趋势策略。
    ...
    进场条件（做多）：价格>ZLEMA 且 斜率>0 且 ADX>20 且 CMF>0
    进场条件（做空）：价格<ZLEMA 且 斜率<0 且 ADX>20 且 CMF<0

    出场条件：ATR追踪止损 / 分层止盈 / 价格穿越ZLEMA+ADX弱化

    优点：ZLEMA修正滞后，ADX过滤震荡
    缺点：1h级别ADX波动大
    """
    name = "i_alltime_v37"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        adx_val = self._adx[i]
        cmf_val = self._cmf[i]
        if np.isnan(zl) or np.isnan(adx_val) or np.isnan(cmf_val):
            return
        zl_slope = 0.0
        ...
        # 3. Signal-based exit
        if side == 1 and (price < zl and adx_val < self.adx_threshold * 0.7):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and (price > zl and adx_val < self.adx_threshold * 0.7):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if price > zl and zl_slope > 0 and adx_val > self.adx_threshold and cmf_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < zl and zl_slope < 0 and adx_val > self.adx_threshold and cmf_val < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
