...
class StrategyV142(TimeSeriesStrategy):
    """
    MTF: daily ADX+DI direction + 4h MACD cross entry.
    Large TF: +DI > -DI & ADX strong. Small TF: MACD cross.
    Strengths: ADX confirms trend, MACD timing. Weaknesses: MACD lags.
    """
    name = "i_alltime_v142"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not (pdi > mdi and adx_v > self.adx_threshold):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (mdi > pdi and adx_v > self.adx_threshold):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (pdi > mdi and adx_v > self.adx_threshold) and (ml_p <= ms_p and ml > ms):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (mdi > pdi and adx_v > self.adx_threshold) and (ml_p >= ms_p and ml < ms):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
