...
class StrategyV146(TimeSeriesStrategy):
    """
    MTF: daily PSAR direction + 4h ROC entry.
    Large TF: close > PSAR. Small TF: ROC > threshold.
    Strengths: PSAR clean, ROC quick. Weaknesses: PSAR whipsaws.
    """
    name = "i_alltime_v146"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (cl > psar_val) and (roc_val > self.roc_threshold):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < psar_val) and (roc_val < -self.roc_threshold):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
