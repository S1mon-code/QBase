...
class StrategyV150(TimeSeriesStrategy):
    """
    MTF: daily Donchian direction + 4h Force Index entry.
    Large TF: close > Don mid. Small TF: FI > 0.
    Strengths: Donchian robust, FI price+vol. Weaknesses: Donchian lags, FI noisy.
    """
    name = "i_alltime_v150"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (cl > d_mid) and (fi_val > self.fi_threshold):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < d_mid) and (fi_val < -self.fi_threshold):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
