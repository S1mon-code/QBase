...
class StrategyV137(TimeSeriesStrategy):
    """
    MTF: 4h Ichimoku cloud + 1h PPO + Klinger entry.
    Large TF: above cloud. Small TF: PPO cross + Klinger confirm.
    Strengths: comprehensive. Weaknesses: many parameters.
    """
    name = "i_alltime_v137"
    freq = "1h"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(pl) or np.isnan(ps) or np.isnan(kl_v) or np.isnan(ks_v):
            return
        if i < 1:
            return
        ...
        # 4. Entry
        if side == 0:
            if (cl > cloud_top) and (pl_p <= ps_p and pl > ps and kl_v > ks_v):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < cloud_bot) and (pl_p >= ps_p and pl < ps and kl_v < ks_v):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
