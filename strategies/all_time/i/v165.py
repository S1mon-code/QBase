...
class StrategyV165(TimeSeriesStrategy):
    """
    策略简介：HMM regime + PPO histogram momentum + Volume Spike entry filter.

    使用指标：
    - HMM Regime(120): market state
    - PPO(12,26,9): percentage price oscillator histogram for momentum
    - Volume Spike(20,1.5): confirms breakout with above-average volume
    - ATR(14): stop distance

    进场条件（做多）：Regime=1 且 PPO hist > 0 且 Volume Spike detected
    进场条件（做空）：Regime=2 且 PPO hist < 0 且 Volume Spike detected

    出场条件：ATR trailing stop / tiered profit / regime reversal

    优点：PPO is percentage-based making it comparable across price levels
    缺点：Volume spike requirement may filter out valid trend continuation entries
    """
    name = "i_alltime_v165"
    warmup = 400
    freq = "daily"
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and ppo_h > 0 and vs:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and ppo_h < 0 and vs:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
