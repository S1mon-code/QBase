...
class StrategyV99(TimeSeriesStrategy):
    """
    策略简介：Keltner Channel突破 + Supertrend方向确认 + MFI资金流的4h突破策略。

    使用指标：
    - Keltner(20, 2.0): 通道突破信号
    - Supertrend(10, 3.0): 趋势方向确认
    - MFI(14): 资金流向确认
    - ATR(14): 止损距离

    进场条件（做多）：价格>KC上轨 + Supertrend direction=1 + MFI>50
    进场条件（做空）：价格<KC下轨 + Supertrend direction=-1 + MFI<50

    出场条件：ATR追踪止损 / 分层止盈 / Supertrend反转

    优点：KC+Supertrend双重通道确认，MFI量价验证
    缺点：双重通道指标可能过度过滤
    """
    name = "i_alltime_v99"
    warmup = 400
    ...
    def on_bar(self, context):
        ...
        st_d = self._st_dir[i]
        mfi_val = self._mfi[i]
        if np.isnan(atr_val) or np.isnan(kc_up) or np.isnan(kc_low) or np.isnan(st_d) or np.isnan(mfi_val):
            return
        ...
        # ── 4. 入场 ──
        if side == 0:
            if price > kc_up and st_d == 1 and mfi_val > 50:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < kc_low and st_d == -1 and mfi_val < 50:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
