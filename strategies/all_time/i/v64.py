...
class StrategyV64(TimeSeriesStrategy):
    """
    RSI(7) fast mean reversion + Bollinger + CMF for Iron Ore (1h).

    Indicators:
    - RSI(7): fast RSI for quick oversold/overbought detection on 1h
    - Bollinger Bands(20,2): price envelope confirmation
    - CMF(20): money flow divergence (smart money vs price)

    Entry (Long): RSI(7) < 15 + price < lower BB + CMF > 0
    Entry (Short): RSI(7) > 85 + price > upper BB + CMF < 0
    Exit: RSI(7) crosses 50

    Pros: Fast RSI catches quick intraday reversals; CMF divergence adds edge
    Cons: RSI(7) very sensitive, may whipsaw on 1h
    """
    name = "i_alltime_v64"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(rsi_val) or np.isnan(bb_u) or np.isnan(cmf_val) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and rsi_val < self.rsi_oversold and c < bb_l and cmf_val > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and rsi_val > self.rsi_overbought and c > bb_u and cmf_val < 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, rsi_val < 30):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, rsi_val > 70):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
