...
class StrategyV97(TimeSeriesStrategy):
    """
    策略简介：Chandelier Exit突破 + PPO动量 + Volume Spike量能飙升的4h突破策略。

    使用指标：
    - Chandelier Exit(22, 3.0): ATR-based exit线突破入场
    - PPO(12,26,9): 百分比价格振荡器方向
    - Volume Spike: 成交量飙升确认
    - ATR(14): 止损距离

    进场条件（做多）：价格>Chandelier Short Exit + PPO line>signal + Vol Spike>mult
    进场条件（做空）：价格<Chandelier Long Exit + PPO line<signal + Vol Spike>mult

    出场条件：ATR追踪止损 / 分层止盈 / PPO方向反转

    优点：Chandelier自适应波动率，PPO消除绝对值影响
    缺点：Vol Spike条件限制交易频率
    """
    name = "i_alltime_v97"
    warmup = 400
    freq = "4h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._ppo_line, self._ppo_signal, _ = ppo(closes, 12, 26, 9)
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or np.isnan(cl) or np.isnan(cs) or np.isnan(pl) or np.isnan(ps) or np.isnan(vs):
            return
        ...
        # ── 4. 入场 ──
        if side == 0:
            if price > cs and pl > ps and vs > self.vol_spike_mult:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < cl and pl < ps and vs > self.vol_spike_mult:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
