...
class StrategyV26(TimeSeriesStrategy):
    """
    策略简介：Supertrend方向 + RSI动量过滤 + OBV量价确认的1h趋势策略。

    使用指标：
    - Supertrend(10,2.5): 趋势方向
    - RSI(14): 动量区间过滤，避免超买超卖入场
    - OBV: 量价方向确认

    进场条件（做多）：Supertrend=1 且 RSI在55-75之间 且 OBV上升
    进场条件（做空）：Supertrend=-1 且 RSI在25-45之间 且 OBV下降

    出场条件：ATR追踪止损 / 分层止盈 / Supertrend反转

    优点：RSI过滤极端区域入场，OBV确认量价一致
    缺点：1h级别噪音较多，Supertrend可能频繁翻转
    """
    name = "i_alltime_v26"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        rsi_val = self._rsi[i]
        obv_val = self._obv[i]
        if np.isnan(st_dir) or np.isnan(rsi_val) or np.isnan(obv_val):
            return
        obv_slope = 0.0
        ...
        # 4. Entry
        if side == 0:
            if st_dir == 1 and rsi_val > self.rsi_threshold and rsi_val < 75 and obv_slope > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif st_dir == -1 and rsi_val < (100 - self.rsi_threshold) and rsi_val > 25 and obv_slope < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
