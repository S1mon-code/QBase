...
class AllTimeIV202(TimeSeriesStrategy):
    """
    策略简介：Supertrend方向 + 趋势强度复合过滤的日线策略。
    使用指标：Supertrend(10,3.0)方向信号，Trend Strength Composite过滤。
    进场条件：Supertrend方向=1且趋势强度>0.5做多；方向=-1且强度>0.5做空。
    出场条件：ATR追踪止损 + Supertrend方向反转退出。
    优点：趋势强度过滤提高信号质量。
    缺点：强趋势启动初期可能错过入场点。
    """
    name = "i_alltime_v202"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(self._st_dir[i]) or np.isnan(self._ts[i]) or np.isnan(atr_val):
            return
        ...
        if side == 0 and self._st_dir[i] == 1 and self._ts[i] > self.ts_threshold:
        ...
        elif side == 0 and self._st_dir[i] == -1 and self._ts[i] > self.ts_threshold:
            lot_size = self._calc_lots(context, price, atr_val)
            if lot_size > 0:
