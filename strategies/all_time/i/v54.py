...
class StrategyV54(TimeSeriesStrategy):
    """
    Detrended Price + Stochastic + OBV mean-reversion for Iron Ore (4h).

    Indicators:
    - DPO(20): isolates cyclical price component by removing trend
    - Stochastic(14,3): timing oscillator for oversold/overbought
    - OBV: cumulative volume trend for accumulation/distribution

    Entry (Long): DPO < -dpo_thresh + Stochastic %K < 20 + OBV rising
    Entry (Short): DPO > +dpo_thresh + Stochastic %K > 80 + OBV falling
    Exit: DPO crosses zero (cycle mean)

    Pros: DPO + Stochastic gives dual cycle confirmation
    Cons: DPO shift means latest values may lag actual price
    """
    name = "i_alltime_v54"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        sk = self._stoch_k[i]
        obv_val = self._obv[i]
        obv_prev = self._obv[max(0, i - 12)] if i >= 12 else np.nan
        atr_val = self._atr[i]
        if np.isnan(dpo_val) or np.isnan(sk) or np.isnan(obv_val) or np.isnan(obv_prev) or np.isnan(atr_val):
            return
        ...
        if side == 0 and dpo_val < -self.dpo_threshold and sk < self.stoch_oversold and obv_rising:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and dpo_val > self.dpo_threshold and sk > self.stoch_overbought and obv_falling:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, dpo_val < 0 and sk < 40):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, dpo_val > 0 and sk > 60):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
