...
class StrategyV186(TimeSeriesStrategy):
    """
    策略简介：Vote(Supertrend, RSI>50/<50, MACD hist) >= 2/3 + Klinger on 1h.

    使用指标：
    - Supertrend(10,3): trend direction
    - RSI(14): momentum above/below 50
    - MACD(12,26,9): histogram direction
    - Klinger(34,55,13): volume oscillator confirmation
    - ATR(14): stop distance

    进场条件（做多）：>= 2/3 bullish votes 且 Klinger > signal
    进场条件（做空）：>= 2/3 bearish votes 且 Klinger < signal

    出场条件：ATR trailing stop / tiered profit / vote reversal

    优点：Combines trend, momentum and flow for robust signals
    缺点：1h frequency generates more noise than 4h
    """
    name = "i_alltime_v186"
    warmup = 600
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        if np.isnan(st) or np.isnan(rv) or np.isnan(mh) or np.isnan(kl) or np.isnan(ks):
            return
        ...
        vol_ok = (kl > ks) if bull_votes >= 2 else ((kl < ks) if bear_votes >= 2 else False)
