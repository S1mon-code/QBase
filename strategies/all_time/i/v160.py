...
class StrategyV160(TimeSeriesStrategy):
    """
    MTF: daily Donchian+Aroon + 4h Stochastic+CMF entry.
    Large TF: close > Don mid + Aroon bullish. Small TF: Stoch oversold + CMF > 0.
    Strengths: dual daily reliable, CMF vol-based. Weaknesses: very restrictive.
    """
    name = "i_alltime_v160"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        sk = self._stoch_k[i]
        cmf_val = self._cmf[i]
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(sk) or np.isnan(cmf_val):
            return
        j = self._lg_map[i]
        ...
        if np.isnan(d_mid) or np.isnan(cl) or np.isnan(a_up) or np.isnan(a_dn):
            return
        ...
        if side == 1 and not (cl > d_mid and a_up > a_dn):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (cl < d_mid and a_dn > a_up):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (cl > d_mid and a_up > a_dn) and (sk < self.stoch_entry and cmf_val > 0):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (cl < d_mid and a_dn > a_up) and (sk > (100 - self.stoch_entry) and cmf_val < 0):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
