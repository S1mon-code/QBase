...
class StrategyV52(TimeSeriesStrategy):
    """
    CMO(-50/+50) reversal + Keltner + Volume Spike mean-reversion for Iron Ore (4h).

    Indicators:
    - CMO(14): Chande Momentum Oscillator; <-50 oversold, >+50 overbought
    - Keltner Channel(20, 1.5): volatility-adaptive bands
    - Volume Spike(20, 2.0): detects abnormal volume for capitulation/climax

    Entry (Long): CMO < -cmo_threshold + price < lower KC + volume spike
    Entry (Short): CMO > +cmo_threshold + price > upper KC + volume spike
    Exit: CMO crosses zero

    Pros: CMO is bounded and symmetric; volume spikes mark exhaustion well
    Cons: CMO threshold sensitivity varies with market regime
    """
    name = "i_alltime_v52"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(cmo_val) or np.isnan(kc_u) or np.isnan(vs) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and cmo_val < -self.cmo_threshold and c < kc_l and vs > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and cmo_val > self.cmo_threshold and c > kc_u and vs > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, cmo_val < -25):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, cmo_val > 25):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
