...
class StrategyV129(TimeSeriesStrategy):
    """
    MTF: 4h MACD histogram direction + 1h Stochastic RSI entry.
    Large TF: MACD hist > 0 = bullish. Small TF: StochRSI oversold.
    Strengths: MACD captures momentum, StochRSI sensitive. Weaknesses: double-smoothed lag.
    """
    name = "i_alltime_v129"
    freq = "1h"
    warmup = 800
    ...
    def on_init_arrays(self, context, bars):
        ...
        n = len(closes)
        self._stoch_k, _ = stoch_rsi(closes, self.stoch_rsi_period)
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (mh > 0) and (sk < self.stoch_entry):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (mh < 0) and (sk > (100 - self.stoch_entry)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
