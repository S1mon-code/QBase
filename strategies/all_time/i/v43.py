...
class StrategyV43(TimeSeriesStrategy):
    """
    Z-score(20) of price + CCI + Volume Spike mean-reversion for Iron Ore.

    Indicators:
    - Z-score(20): standardized deviation from 20-bar mean; |z|>2 = extreme
    - CCI(20): commodity channel index; <-100 oversold, >+100 overbought
    - Volume Spike(20, 2.0): confirms capitulation/climax volume at extremes

    Entry (Long):
    - Z-score < -zscore_threshold (price well below mean)
    - CCI < -100 (oversold)
    - Volume spike detected (capitulation selling)

    Entry (Short):
    - Z-score > +zscore_threshold (price well above mean)
    - CCI > +100 (overbought)
    - Volume spike detected (climax buying)

    Exit:
    - Z-score returns to 0 (mean reversion complete)
    - ATR trailing stop / tiered profit-taking

    Pros: Z-score is statistically rigorous; volume spikes mark exhaustion points
    Cons: Z-score assumes normal distribution which commodities may violate
    """
    name = "i_alltime_v43"
    warmup = 250
    freq = "daily"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(z) or np.isnan(cci_val) or np.isnan(spike) or np.isnan(atr_val):
            return
        ...
        # 3. Signal exit — z-score returns to 0
        if side == 1 and z >= 0:
            context.close_long()
            self._reset_state()
            return
        ...
        # 4. Entry - Long
        if side == 0 and z < -self.zscore_threshold and cci_val < -100 and spike > 0:
        ...
        elif side == 0 and z > self.zscore_threshold and cci_val > 100 and spike > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
