...
class StrategyV139(TimeSeriesStrategy):
    """
    MTF: 4h Vortex + 1h Williams %%R + Force Index entry.
    Large TF: +VI > -VI. Small TF: WR oversold + FI > 0.
    Strengths: WR fast, FI vol-confirm. Weaknesses: WR noisy, FI spike-driven.
    """
    name = "i_alltime_v139"
    freq = "1h"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        wr_val = self._wr[i]
        fi_val = self._fi[i]
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(wr_val) or np.isnan(fi_val):
            return
        j = self._lg_map[i]
        ...
        if side == 1 and not (pvi > mvi):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (mvi > pvi):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (pvi > mvi) and (wr_val < self.wr_entry and fi_val > 0):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (mvi > pvi) and (wr_val > -(100 + self.wr_entry) and fi_val < 0):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
