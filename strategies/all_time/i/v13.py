...
class StrategyV13(TimeSeriesStrategy):
    """
    策略简介：TEMA三重指数均线 + PPO标准化动量 + 成交量动量的4h趋势策略。

    使用指标：
    - TEMA(20): 三重指数均线，极低延迟
    - PPO(12,26,9): 百分比价格振荡器，标准化便于跨品种
    - Volume Momentum(10): 成交量变化动量

    进场条件（做多）：价格>TEMA 且 TEMA斜率>0 且 PPO线>信号线 且 量能动量>0
    进场条件（做空）：价格<TEMA 且 TEMA斜率<0 且 PPO线<信号线 且 量能动量>0

    出场条件：ATR追踪止损 / 分层止盈 / TEMA+PPO双反转

    优点：TEMA极低延迟捕捉趋势转折，PPO标准化跨品种一致
    缺点：TEMA对噪音敏感，震荡市容易产生假信号
    """
    name = "i_alltime_v13"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        vm = self._vol_mom[i]
        if np.isnan(tema_val) or np.isnan(ppo_l) or np.isnan(ppo_s) or np.isnan(vm):
            return
        tema_slope = 0.0
        if i >= 1 and not np.isnan(self._tema[i - 1]):
            tema_slope = tema_val - self._tema[i - 1]
        ...
        # 3. Signal-based exit
        if side == 1 and (tema_slope < 0 and ppo_l < ppo_s):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and (tema_slope > 0 and ppo_l > ppo_s):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if price > tema_val and tema_slope > 0 and ppo_l > ppo_s and vm > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < tema_val and tema_slope < 0 and ppo_l < ppo_s and vm > 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
