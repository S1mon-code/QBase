...
class StrategyV119(TimeSeriesStrategy):
    """
    策略简介：NR7窄幅 + TSI趋势强度 + Volume Momentum的1h突破策略。

    使用指标：NR7, TSI(25,13), Volume Momentum, ATR(14)
    进场条件（做多）：近期NR7 + TSI>0 + Vol Mom>0
    进场条件（做空）：近期NR7 + TSI<0 + Vol Mom<0
    出场条件：ATR追踪止损 / 分层止盈 / TSI反向
    优点：NR7精确识别盘整，TSI平滑趋势
    缺点：TSI参数敏感
    """
    name = "i_alltime_v119"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        tsi_val = self._tsi[i]
        vm_val = self._vm[i]
        if np.isnan(atr_val) or np.isnan(tsi_val) or np.isnan(vm_val):
            return
        ...
        # ── 4. 入场 ──
        if side == 0:
            if recent_nr7 and tsi_val > 0 and vm_val > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif recent_nr7 and tsi_val < 0 and vm_val < 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
