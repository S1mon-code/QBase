...
class StrategyV65(TimeSeriesStrategy):
    """
    Stochastic(5,3) + Keltner + Klinger mean-reversion for Iron Ore (1h).

    Indicators:
    - Stochastic(5,3): fast stochastic for quick reversal timing
    - Keltner Channel(20,1.5): ATR-based bands
    - Klinger Oscillator: volume-based trend/reversal signal

    Entry (Long): Stoch %K < 15 + price < lower KC + Klinger > signal
    Entry (Short): Stoch %K > 85 + price > upper KC + Klinger < signal
    Exit: Stoch %K crosses 50

    Pros: Fast stochastic on 1h gives precise reversal timing
    Cons: Very fast stochastic may generate excessive signals
    """
    name = "i_alltime_v65"
    warmup = 800
    freq = "1h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._stoch_k, _ = stochastic(highs, lows, closes, k_period=5, d_period=3)
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(sk) or np.isnan(kc_u) or np.isnan(kl) or np.isnan(ks) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and sk < self.stoch_oversold and c < kc_l and kl > ks:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and sk > self.stoch_overbought and c > kc_u and kl < ks:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, sk < 25):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, sk > 75):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
