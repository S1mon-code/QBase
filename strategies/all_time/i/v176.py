...
class StrategyV176(TimeSeriesStrategy):
    """
    策略简介：Kalman slope + ROC momentum + Volume Spike on 1h.

    使用指标：
    - Kalman Filter: adaptive trend slope
    - ROC(20): rate of change momentum
    - Volume Spike(20,1.5): above-average volume detection
    - ATR(14): stop distance

    进场条件（做多）：Kalman slope > slope_thresh 且 ROC > 0 且 volume spike
    进场条件（做空）：Kalman slope < -slope_thresh 且 ROC < 0 且 volume spike

    出场条件：ATR trailing stop / tiered profit / slope reversal

    优点：Volume spike confirms conviction behind the move
    缺点：1h Kalman slope is noisier than daily
    """
    name = "i_alltime_v176"
    warmup = 600
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        if np.isnan(ks) or np.isnan(roc_val):
            return
        ...
        # 4. Entry
        if side == 0:
            if ks > self.slope_thresh and roc_val > 0 and vs:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif ks < -self.slope_thresh and roc_val < 0 and vs:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
