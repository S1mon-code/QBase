...
class StrategyV62(TimeSeriesStrategy):
    """
    Keltner Channel + Stochastic + Volume Spike mean-reversion for Iron Ore (1h).

    Indicators:
    - Keltner(20,1.5): ATR-based adaptive bands
    - Stochastic(14,3): oversold/overbought timing
    - Volume Spike(20,2): capitulation/climax detection

    Entry (Long): Price < lower KC + Stoch %K < 20 + volume spike
    Entry (Short): Price > upper KC + Stoch %K > 80 + volume spike
    Exit: Price returns to KC middle

    Pros: Volume spikes on 1h are cleaner signals than daily spikes
    Cons: Keltner bands may be too tight on 1h with high volatility
    """
    name = "i_alltime_v62"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(kc_u) or np.isnan(sk) or np.isnan(vs) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and c < kc_l and sk < self.stoch_oversold and vs > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and c > kc_u and sk > self.stoch_overbought and vs > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
