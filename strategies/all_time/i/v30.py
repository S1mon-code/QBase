...
class StrategyV30(TimeSeriesStrategy):
    """
    策略简介：HMA低延迟均线 + Stochastic动量 + MFI资金流量的1h趋势策略。

    使用指标：
    - HMA(14): Hull均线，低延迟
    - Stochastic(14,3): K/D确认动量方向
    - MFI(14): 资金流量指标

    进场条件（做多）：HMA斜率>0 且 K>D 且 K>50 且 MFI>50
    进场条件（做空）：HMA斜率<0 且 K<D 且 K<50 且 MFI<50

    出场条件：ATR追踪止损 / 分层止盈 / HMA斜率反转或Stoch极端

    优点：HMA低延迟，Stochastic+MFI双重量价确认
    缺点：1h级别噪音大，需严格止损
    """
    name = "i_alltime_v30"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        sd = self._stoch_d[i]
        mfi_val = self._mfi[i]
        if np.isnan(hma_val) or np.isnan(sk) or np.isnan(sd) or np.isnan(mfi_val):
            return
        hma_slope = 0.0
        ...
        # 3. Signal-based exit
        if side == 1 and (hma_slope < 0 or (sk < 30 and sd < 30)):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and (hma_slope > 0 or (sk > 70 and sd > 70)):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if hma_slope > 0 and sk > sd and sk > self.stoch_threshold and mfi_val > 50:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif hma_slope < 0 and sk < sd and sk < (100 - self.stoch_threshold) and mfi_val < 50:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
