...
class StrategyV94(TimeSeriesStrategy):
    """
    策略简介：Donchian(20)通道突破 + CCI动量确认 + Klinger量能振荡的4h突破策略。

    使用指标：
    - Donchian(20): 通道突破信号
    - CCI(14): >100做多确认，<-100做空确认
    - Klinger: line>signal确认买方量能，反之卖方
    - ATR(14): 止损距离

    进场条件（做多）：价格>Donchian上轨 + CCI>100 + Klinger line>signal
    进场条件（做空）：价格<Donchian下轨 + CCI<-100 + Klinger line<signal

    出场条件：ATR追踪止损 / 分层止盈 / CCI回归零轴

    优点：CCI对突破强度敏感，Klinger量能确认
    缺点：CCI>100门槛较高，可能错过温和突破
    """
    name = "i_alltime_v94"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or np.isnan(don_up) or np.isnan(don_low) or np.isnan(cci_val) or np.isnan(kl) or np.isnan(ks):
            return
        ...
        # ── 4. 入场 ──
        if side == 0:
            if price > don_up and cci_val > self.cci_threshold and kl > ks:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < don_low and cci_val < -self.cci_threshold and kl < ks:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
