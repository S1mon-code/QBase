...
class StrategyV67(TimeSeriesStrategy):
    """
    CMO + Bollinger + OI Momentum mean-reversion for Iron Ore (1h).

    Indicators:
    - CMO(14): Chande Momentum Oscillator, bounded -100/+100
    - Bollinger Bands(20,2): price envelope
    - OI Momentum(10): open interest change rate

    Entry (Long): CMO < -50 + price < lower BB + OI mom > 0
    Entry (Short): CMO > +50 + price > upper BB + OI mom > 0
    Exit: CMO crosses zero

    Pros: OI momentum on 1h catches new position building at extremes
    Cons: OI data granularity may not match 1h bars precisely
    """
    name = "i_alltime_v67"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(cmo_val) or np.isnan(bb_u) or np.isnan(oi_m) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and cmo_val < -self.cmo_threshold and c < bb_l and oi_m > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and cmo_val > self.cmo_threshold and c > bb_u and oi_m > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
