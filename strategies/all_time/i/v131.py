...
class StrategyV131(TimeSeriesStrategy):
    """
    MTF: 4h Supertrend direction + 1h Bollinger mean reversion entry.
    Large TF: ST bullish. Small TF: price < lower BB (long), > upper BB (short).
    Strengths: trend-aligned reversion. Weaknesses: BB compressed = missed entries.
    """
    name = "i_alltime_v131"
    freq = "1h"
    warmup = 800
    ...
    def on_init_arrays(self, context, bars):
        ...
        n = len(closes)
        u, _, lo = bollinger_bands(closes, self.bb_period, self.bb_mult)
        self._bb_upper = u
        self._bb_lower = lo
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(bb_u) or np.isnan(bb_l):
            return
        j = self._lg_map[i]
        ...
        if side == 1 and not (st_dir == 1):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (st_dir == -1):
            context.close_short()
            self._reset_state()
            return
