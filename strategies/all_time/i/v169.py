...
class StrategyV169(TimeSeriesStrategy):
    """
    策略简介：HMM regime + Stochastic timing + Volume Momentum confirmation on 4h.

    使用指标：
    - HMM Regime(120): market regime
    - Stochastic(14,3): overbought/oversold oscillator
    - Volume Momentum(14): ratio of volume to its moving average
    - ATR(14): stop distance

    进场条件（做多）：Regime=1 且 Stoch %K < stoch_os 且 Volume Momentum > 1.0
    进场条件（做空）：Regime=2 且 Stoch %K > stoch_ob 且 Volume Momentum > 1.0

    出场条件：ATR trailing stop / tiered profit / regime change

    优点：Volume momentum confirms increasing participation
    缺点：Volume momentum threshold is arbitrary
    """
    name = "i_alltime_v169"
    warmup = 400
    freq = "4h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._vol_mom = volume_momentum(volumes, 14)
    ...
    def on_bar(self, context):
        ...
        vm = self._vol_mom[i]
        if np.isnan(stoch_val) or np.isnan(vm):
            return
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and stoch_val < self.stoch_os and vm > 1.0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and stoch_val > self.stoch_ob and vm > 1.0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
