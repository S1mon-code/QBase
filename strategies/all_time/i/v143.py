...
class StrategyV143(TimeSeriesStrategy):
    """
    MTF: daily Aroon direction + 4h Stochastic entry.
    Large TF: Aroon Up > Down. Small TF: Stoch oversold.
    Strengths: Aroon trend age, Stoch pullback. Weaknesses: Aroon choppy.
    """
    name = "i_alltime_v143"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not (a_up > a_dn and a_up > self.aroon_threshold):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and not (a_dn > a_up and a_dn > self.aroon_threshold):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if (a_up > a_dn and a_up > self.aroon_threshold) and (stoch_k < self.stoch_entry):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (a_dn > a_up and a_dn > self.aroon_threshold) and (stoch_k > (100 - self.stoch_entry)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
