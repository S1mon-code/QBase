...
class StrategyV80(TimeSeriesStrategy):
    """
    Price vs VWAP + Stochastic + CMF mean-reversion for Iron Ore (1h).

    Indicators:
    - VWAP: volume-weighted average price as fair value reference
    - Stochastic(14,3): timing oscillator for entry precision
    - CMF(20): money flow confirms positioning

    Entry (Long): Price < VWAP * (1 - dev) + Stoch %K < 20 + CMF > 0
    Entry (Short): Price > VWAP * (1 + dev) + Stoch %K > 80 + CMF < 0
    Exit: Price returns to VWAP

    Pros: VWAP is the institutional fair value; Stoch+CMF add timing
    Cons: VWAP resets may cause false signals at session boundaries
    """
    name = "i_alltime_v80"
    warmup = 800
    freq = "1h"

    vwap_dev: float = 0.015
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(vwap_val) or np.isnan(sk) or np.isnan(cmf_val) or np.isnan(atr_val) or vwap_val < 1e-9:
            return
        ...
        # 4. Entry
        if side == 0 and c < vwap_val * (1 - self.vwap_dev) and sk < self.stoch_oversold and cmf_val > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and c > vwap_val * (1 + self.vwap_dev) and sk > self.stoch_overbought and cmf_val < 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, sk < 30):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, sk > 70):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
