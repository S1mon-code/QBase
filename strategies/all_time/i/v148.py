...
class StrategyV148(TimeSeriesStrategy):
    """
    MTF: daily HMA direction + 4h Stochastic RSI entry.
    Large TF: HMA rising. Small TF: StochRSI oversold.
    Strengths: HMA responsive, StochRSI sensitive. Weaknesses: HMA overshoot.
    """
    name = "i_alltime_v148"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (hma_now > hma_prev) and (sk < self.stoch_entry):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (hma_now < hma_prev) and (sk > (100 - self.stoch_entry)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
