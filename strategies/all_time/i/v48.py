...
class StrategyV48(TimeSeriesStrategy):
    """
    CCI(-100/+100) reversal + Bollinger + Volume Momentum for Iron Ore (4h).

    Indicators:
    - CCI(20): commodity channel index; <-100 oversold, >+100 overbought
    - Bollinger Bands(20,2): price envelope for extreme detection
    - Volume Momentum(10): rate of change of volume; spikes confirm exhaustion

    Entry (Long): CCI < -100 + price < lower BB + volume momentum positive
    Entry (Short): CCI > +100 + price > upper BB + volume momentum positive
    Exit: CCI returns to zero zone (-50 to +50)

    Pros: CCI mean-reverts naturally; volume momentum confirms capitulation
    Cons: CCI can stay extreme in strong trends
    """
    name = "i_alltime_v48"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(cci_val) or np.isnan(bb_u) or np.isnan(vm) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and cci_val < -self.cci_threshold and c < bb_l and vm > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and cci_val > self.cci_threshold and c > bb_u and vm > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
