...
class StrategyV147(TimeSeriesStrategy):
    """
    MTF: daily Vortex direction + 4h Williams %%R entry.
    Large TF: +VI > -VI. Small TF: WR oversold.
    Strengths: daily Vortex stable, WR fast. Weaknesses: Vortex choppy, WR noisy.
    """
    name = "i_alltime_v147"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        wr_val = self._wr[i]
        if np.isnan(atr_val) or atr_val <= 0 or np.isnan(wr_val):
            return
        j = self._lg_map[i]
        ...
        # 4. Entry
        if side == 0:
            if (pvi > mvi) and (wr_val < self.wr_entry):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (mvi > pvi) and (wr_val > -(100 + self.wr_entry)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
