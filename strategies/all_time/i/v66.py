...
class StrategyV66(TimeSeriesStrategy):
    """
    CCI + Williams %R + Volume Momentum mean-reversion for Iron Ore (1h).

    Indicators:
    - CCI(20): measures price deviation from statistical mean
    - Williams %R(14): bounded oscillator for timing
    - Volume Momentum(10): rate of change of volume

    Entry (Long): CCI < -150 + Williams %R < -85 + vol momentum > 0
    Entry (Short): CCI > +150 + Williams %R > -15 + vol momentum > 0
    Exit: CCI returns to zero zone

    Pros: Dual oscillator confirmation reduces false signals on 1h
    Cons: Both CCI and %R may be redundant in some regimes
    """
    name = "i_alltime_v66"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        vm = self._vol_mom[i]
        atr_val = self._atr[i]
        if np.isnan(cci_val) or np.isnan(wr_val) or np.isnan(vm) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and cci_val < -self.cci_threshold and wr_val < self.wr_oversold and vm > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and cci_val > self.cci_threshold and wr_val > self.wr_overbought and vm > 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
        ...
        # 5. Scale-in
        elif side == 1 and self._should_add_long(price, atr_val, cci_val < -75):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
                context.buy(add_lots)
                self.position_scale += 1
                self.bars_since_last_scale = 0
        elif side == -1 and self._should_add_short(price, atr_val, cci_val > 75):
            add_lots = self._calc_add_lots(self._calc_lots(context, atr_val))
            if add_lots > 0:
