...
class StrategyV79(TimeSeriesStrategy):
    """
    Detrended Price + Fisher Transform + MFI mean-reversion for Iron Ore (1h).

    Indicators:
    - DPO(20): cycle isolation; removes trend for pure cycle trading
    - Fisher Transform(10): Gaussian-normalized extremes
    - MFI(14): volume-weighted momentum

    Entry (Long): DPO < -thresh + Fisher < -1.0 + MFI < 25
    Entry (Short): DPO > +thresh + Fisher > +1.0 + MFI > 75
    Exit: DPO crosses zero

    Pros: DPO + Fisher gives dual cycle/statistical reversal signal
    Cons: DPO lag + Fisher sensitivity may conflict on 1h
    """
    name = "i_alltime_v79"
    warmup = 800
    freq = "1h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._fisher = fisher_transform(highs, lows, period=10)
    ...
    def on_bar(self, context):
        ...
        mfi_val = self._mfi[i]
        atr_val = self._atr[i]
        if np.isnan(dpo_val) or np.isnan(fv) or np.isnan(mfi_val) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and dpo_val < -self.dpo_threshold and fv < -self.fisher_threshold and mfi_val < self.mfi_threshold:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and dpo_val > self.dpo_threshold and fv > self.fisher_threshold and mfi_val > (100 - self.mfi_threshold):
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
