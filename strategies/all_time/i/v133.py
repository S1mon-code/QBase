...
class StrategyV133(TimeSeriesStrategy):
    """
    MTF: 4h Linear Regression slope + 1h RSI + Volume Spike entry.
    Large TF: LR slope > 0 = bullish. Small TF: RSI pullback + volume spike.
    Strengths: LR slope clean, vol confirms. Weaknesses: LR lagging, vol spikes rare.
    """
    name = "i_alltime_v133"
    freq = "1h"
    warmup = 800
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (slope > 0) and (rsi_val < self.rsi_entry and vs > self.vol_spike_mult):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (slope < 0) and (rsi_val > (100 - self.rsi_entry) and vs > self.vol_spike_mult):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
