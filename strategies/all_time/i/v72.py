...
class StrategyV72(TimeSeriesStrategy):
    """
    Z-score + Williams %R + CMF mean-reversion for Iron Ore (1h).

    Indicators:
    - Z-score(20): statistical deviation from rolling mean
    - Williams %R(14): bounded momentum oscillator
    - CMF(20): money flow for volume confirmation

    Entry (Long): Z-score < -1.8 + Williams %R < -85 + CMF > 0
    Entry (Short): Z-score > +1.8 + Williams %R > -15 + CMF < 0
    Exit: Z-score returns to zero

    Pros: Z-score + CMF divergence = strong reversal signal
    Cons: CMF can be slow to confirm on 1h timeframe
    """
    name = "i_alltime_v72"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        cmf_val = self._cmf[i]
        atr_val = self._atr[i]
        if np.isnan(z) or np.isnan(wr_val) or np.isnan(cmf_val) or np.isnan(atr_val):
            return
        ...
        # 4. Entry
        if side == 0 and z < -self.zscore_threshold and wr_val < self.wr_oversold and cmf_val > 0:
            ...
            if base_lots > 0:
                ...
                self.bars_since_last_scale = 0
                self.direction = 1
        elif side == 0 and z > self.zscore_threshold and wr_val > self.wr_overbought and cmf_val < 0:
            base_lots = self._calc_lots(context, atr_val)
            if base_lots > 0:
