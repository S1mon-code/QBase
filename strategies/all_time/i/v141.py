...
class StrategyV141(TimeSeriesStrategy):
    """
    MTF: daily Supertrend direction + 4h RSI entry.
    Large TF: daily ST bullish. Small TF: 4h RSI oversold pullback.
    Strengths: daily trend stable. Weaknesses: slow to reverse.
    """
    name = "i_alltime_v141"
    freq = "4h"
    warmup = 500
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if (st_dir == 1) and (rsi_val < self.rsi_entry):
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif (st_dir == -1) and (rsi_val > (100 - self.rsi_entry)):
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
