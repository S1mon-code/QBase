...
class StrategyV162(TimeSeriesStrategy):
    """
    策略简介：HMM regime + MACD crossover + OBV trend confirmation.

    使用指标：
    - HMM Regime(120): market state (0=ranging, 1=bull, 2=bear)
    - MACD(12,26,9): momentum signal via histogram sign
    - OBV: volume-weighted price trend confirmation
    - ATR(14): stop distance

    进场条件（做多）：Regime=1 且 MACD histogram > 0 且 OBV rising
    进场条件（做空）：Regime=2 且 MACD histogram < 0 且 OBV falling

    出场条件：ATR trailing stop / tiered profit / regime reversal

    优点：三重确认 (regime + momentum + volume flow)
    缺点：OBV is cumulative and can be noisy
    """
    name = "i_alltime_v162"
    warmup = 400
    freq = "daily"
    ...
    def on_bar(self, context):
        ...
        # 4. Entry
        if side == 0:
            if regime == 1 and macd_h > 0 and obv_rising:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif regime == 2 and macd_h < 0 and not obv_rising:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
