...
class StrategyV14(TimeSeriesStrategy):
    """
    策略简介：McGinley动态均线 + RSI动量 + CMF资金流的4h趋势策略。

    使用指标：
    - McGinley Dynamic(14): 自适应均线，跟踪价格更紧密
    - RSI(14): 动量方向，50以上=多头区
    - CMF(20): 资金流向，>0.05=明显买方压力

    进场条件（做多）：价格>McGinley 且 RSI在50-70之间 且 CMF>0.05
    进场条件（做空）：价格<McGinley 且 RSI在30-50之间 且 CMF<-0.05

    出场条件：ATR追踪止损 / 分层止盈 / 价格穿越McGinley+RSI确认

    优点：McGinley自适应跟踪价格，CMF确认资金方向
    缺点：McGinley在快速反转中可能反应不够快
    """
    name = "i_alltime_v14"
    warmup = 400
    freq = "4h"
    ...
    def on_bar(self, context):
        ...
        rsi_val = self._rsi[i]
        cmf_val = self._cmf[i]
        if np.isnan(mcg_val) or np.isnan(rsi_val) or np.isnan(cmf_val):
            return
        ...
        # 3. Signal-based exit
        if side == 1 and (price < mcg_val and rsi_val < 45):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and (price > mcg_val and rsi_val > 55):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if price > mcg_val and rsi_val > 50 and rsi_val < self.rsi_ob and cmf_val > 0.05:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif price < mcg_val and rsi_val < 50 and rsi_val > (100 - self.rsi_ob) and cmf_val < -0.05:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
