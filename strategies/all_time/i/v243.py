...
class AllTimeIV243(TimeSeriesStrategy):
    """
    策略简介：BB突破 + EMV确认的4h突破策略。
    使用指标：BB(20,2.0)突破，EMV(14)确认价格移动容易度。
    进场条件：价格>BB上轨且EMV>0做多；价格<BB下轨且EMV<0做空。
    出场条件：ATR追踪止损 + 价格回到BB中轨。
    优点：EMV结合价格和成交量评估运动质量。
    缺点：EMV对极端成交量敏感。
    """
    name = "i_alltime_v243"
    warmup = 60
    ...
    def on_bar(self, context):
        ...
        atr_val = self._atr[i]
        if np.isnan(self._bb_upper[i]) or np.isnan(self._bb_lower[i]) or np.isnan(self._emv[i]) or np.isnan(atr_val):
            return
        ...
        if side == 0 and price > self._bb_upper[i] and self._emv[i] > 0:
        ...
        elif side == 0 and price < self._bb_lower[i] and self._emv[i] < 0:
            lot_size = self._calc_lots(context, price, atr_val)
            if lot_size > 0:
