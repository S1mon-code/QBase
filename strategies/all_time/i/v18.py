...
class StrategyV18(TimeSeriesStrategy):
    """
    策略简介：EMA Ribbon趋势 + Stochastic确认 + 放量触发的4h趋势策略。

    使用指标：
    - EMA Ribbon(8,21): 快慢EMA排列定方向，ribbon宽度量化趋势强度
    - Stochastic(14,3): K/D交叉确认短期动量
    - Volume Spike(20,2.0): 放量确认信号有效

    进场条件（做多）：快EMA>慢EMA 且 K>D 且 K<80 且 放量
    进场条件（做空）：快EMA<慢EMA 且 K<D 且 K>20 且 放量

    出场条件：ATR追踪止损 / 分层止盈 / EMA Ribbon交叉

    优点：Ribbon宽度量化趋势强度，Stochastic避免追高，放量确认突破
    缺点：震荡市ribbon频繁交叉产生假信号
    """
    name = "i_alltime_v18"
    warmup = 400
    freq = "4h"
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._ribbon_fast, self._ribbon_slow, self._ribbon_width = ema_ribbon(closes, self.ribbon_fast, self.ribbon_slow)
    ...
    def on_bar(self, context):
        ...
        rf = self._ribbon_fast[i]
        rs = self._ribbon_slow[i]
        rw = self._ribbon_width[i]
        sk = self._stoch_k[i]
        sd = self._stoch_d[i]
        spike = self._vol_spike[i]
        if np.isnan(rf) or np.isnan(rs) or np.isnan(sk) or np.isnan(sd) or np.isnan(spike):
            return
        ...
        # 4. Entry
        if side == 0:
            if rf > rs and rw > 0 and sk > sd and sk < self.stoch_ob and spike > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif rf < rs and rw > 0 and sk < sd and sk > (100 - self.stoch_ob) and spike > 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
