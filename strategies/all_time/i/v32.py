...
class StrategyV32(TimeSeriesStrategy):
    """
    策略简介：线性回归斜率+R² + CCI极值 + 成交量动量的1h趋势策略。

    使用指标：
    - Linear Regression(20): 斜率定方向，R²量化趋势线性度
    - CCI(14): 极值确认强趋势
    - Volume Momentum(10): 量能动量

    进场条件（做多）：LR斜率>0 且 R²>0.4 且 CCI>80 且 量能动量>0
    进场条件（做空）：LR斜率<0 且 R²>0.4 且 CCI<-80 且 量能动量>0

    出场条件：ATR追踪止损 / 分层止盈 / 斜率反转或CCI回归

    优点：R²确保趋势线性，CCI+量能双重确认
    缺点：LR对异常值敏感，1h级别R²波动大
    """
    name = "i_alltime_v32"
    warmup = 800
    freq = "1h"
    ...
    def on_bar(self, context):
        ...
        cci_val = self._cci[i]
        vm = self._vol_mom[i]
        if np.isnan(slope) or np.isnan(r2) or np.isnan(cci_val) or np.isnan(vm):
            return
        ...
        # 3. Signal-based exit
        if side == 1 and (slope < 0 or cci_val < 0):
            context.close_long()
            self._reset_state()
            return
        elif side == -1 and (slope > 0 or cci_val > 0):
            context.close_short()
            self._reset_state()
            return
        ...
        # 4. Entry
        if side == 0:
            if slope > 0 and r2 > self.r2_threshold and cci_val > self.cci_threshold and vm > 0:
                ...
                if base_lots > 0:
                    ...
                    self.bars_since_last_scale = 0
                    self.direction = 1
            elif slope < 0 and r2 > self.r2_threshold and cci_val < -self.cci_threshold and vm > 0:
                base_lots = self._calc_lots(context, atr_val)
                if base_lots > 0:
