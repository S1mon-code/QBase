"""Strong Trend v51 — Volume Momentum + OI Momentum dual confirmation."""
import sys
...
class StrongTrendV51(TimeSeriesStrategy):
    """
    策略简介：Volume Momentum + OI Momentum 双重量价确认的强趋势策略。

    使用指标：
    - Volume Momentum(14): 成交量动量，>2.0 确认放量
    - OI Momentum(20): 持仓量动量，>0 确认资金流入
    - ATR(14): 止损距离计算
    ...
    优点：双重量价确认，过滤假突破
    缺点：需要量和OI同时满足，可能错过快速启动的趋势
    """
    name = "strong_trend_v51"
    warmup = 60
    freq = "daily"
    ...
    def on_bar(self, context):
        ...
        om = self._oi_mom[i]
        atr_val = self._atr[i]
        if np.isnan(vm) or np.isnan(om) or np.isnan(atr_val):
            return
        ...
        # Entry
        if side == 0 and vm > self.vol_threshold and om > 0:
