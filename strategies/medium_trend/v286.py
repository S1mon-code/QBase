...
class MediumTrendV286(TimeSeriesStrategy):
    """
    策略简介：月度季节性模式+ROC动量确认。
    使用指标：Monthly Seasonal(3yr) + ROC(12) + ATR
    进场条件：月度季节性为正且ROC>0
    出场条件：ATR追踪止损 / ROC转负
    优点：历史月度统计+当前动量验证
    缺点：月度模式变化缓慢
    ...
    def on_bar(self, context):
        ...
        roc_val = self._roc[i]
        if np.isnan(atr_val) or np.isnan(seas_val) or np.isnan(roc_val) or atr_val <= 0:
            return
        ...
        if side == 0 and seas_val > 0 and roc_val > 0:
