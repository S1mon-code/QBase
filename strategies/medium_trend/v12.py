...
class StrategyV12(TimeSeriesStrategy):
    """
    策略简介：Hull MA趋势方向 + Chaikin Money Flow量资金确认的做多策略（5min频率）。

    使用指标：
    - HMA(20): 低滞后趋势方向，价格>HMA为多头
    - CMF(20): 资金流入确认，CMF>0表示买方主导
    - ATR(14): 止损距离计算

    进场条件（做多）：价格 > HMA 且 HMA斜率 > 0 且 CMF > 0
    出场条件：ATR追踪止损 / 分层止盈 / 价格跌破HMA
    ...
    def on_bar(self, context):
        ...
        hma_val = self._hma[i]
        cmf_val = self._cmf[i]
        if np.isnan(atr_val) or np.isnan(hma_val) or np.isnan(cmf_val):
            return
        ...
        bullish = price > hma_val and hma_slope > 0 and cmf_val > 0
