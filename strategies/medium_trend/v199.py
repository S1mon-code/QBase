...
class StrategyV199(TimeSeriesStrategy):
    """
    策略简介：Hurst反转Exit+Changepoint入场做多（1h）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_bar(self, context):
        ...
        # Hurst exit: H < 0.45 = mean reverting
        if side == 1:
            h=self._hurst[i]
            if not np.isnan(h) and h<0.45:
                context.close_long()
                self._reset_state()
                return
        ...
        if side == 0 and not np.isnan(self._cp[i]) and self._cp[i]>0.7 and not np.isnan(self._hurst[i]) and self._hurst[i]>0.55:
