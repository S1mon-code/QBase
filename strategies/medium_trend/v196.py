...
class StrategyV196(TimeSeriesStrategy):
    """
    策略简介：MFI超买Exit+CMO动量入场做多（30min）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._mfi = mfi(highs,lows,closes,volumes)
        self._cmo = cmo(closes)
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not np.isnan(self._cmo[i]) and self._cmo[i]<-20:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and not np.isnan(self._cmo[i]) and self._cmo[i]>20:
