...
class StrategyV195(TimeSeriesStrategy):
    """
    策略简介：Williams %R超买Exit+Kalman趋势入场做多（1h）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_init_arrays(self, context, bars):
        ...
        _,self._slope,_ = kalman_filter(closes)
    ...
    def on_bar(self, context):
        ...
        if side == 0 and not np.isnan(self._slope[i]) and self._slope[i]>0:
