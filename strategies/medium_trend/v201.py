...
class MediumTrendV201(TimeSeriesStrategy):
    """
    策略简介：成交量动量 + ADX趋势强度确认的日线做多策略。

    使用指标：
    - Volume Momentum(14): 成交量动量判断资金流入强度
    - ADX(14): 趋势强度确认，ADX > 20时市场存在趋势
    - ATR(14): 止损距离计算

    进场条件（做多）：
    - Volume Momentum > 1.5（成交量放大，资金流入）
    - ADX > 20（市场处于趋势状态）
    ...
    def on_bar(self, context):
        ...
        vm = self._vol_mom[i]
        adx_val = self._adx[i]
        if np.isnan(vm) or np.isnan(adx_val):
            return
        ...
        if side == 1 and adx_val < 15:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and vm > self.vol_mom_entry and adx_val > self.adx_threshold:
