...
class MediumTrendV207(TimeSeriesStrategy):
    """
    策略简介：MFI资金流指数 + Vortex趋势方向的日线做多策略。

    使用指标：
    - MFI(14): 资金流指数，综合量价的RSI变体
    - Vortex(14): VI+/VI-交叉判定趋势方向
    - ATR(14): 止损距离计算

    进场条件（做多）：
    - MFI从 < 30回升至 > 30（超卖回升，资金重新流入）
    - VI+ > VI-（上升趋势力量占优）
    ...
    def on_bar(self, context):
        ...
        if np.isnan(mfi_val) or np.isnan(vip) or np.isnan(vim):
            return
        ...
        if side == 0 and prev_mfi < self.mfi_entry and mfi_val > self.mfi_entry and vip > vim:
