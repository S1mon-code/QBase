...
class StrategyV187(TimeSeriesStrategy):
    """
    策略简介：Regime条件止损+Ensemble入场做多（日线）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_init_arrays(self, context, bars):
        ...
        _,self._it,_ = composite_regime(closes, highs, lows, period=20)
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not np.isnan(self._vote[i]) and self._vote[i]<-0.3:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and not np.isnan(self._vote[i]) and self._vote[i]>0.3 and not np.isnan(self._it[i]) and self._it[i]==1:
