...
class StrategyV188(TimeSeriesStrategy):
    """
    策略简介：Ratcheting ATR(每10%收紧)+Boosting入场做多（日线）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_bar(self, context):
        ...
        # Ratcheting: tighten per 10% gain
        if side == 1 and self.entry_price > 0:
            gp = (price - self.entry_price) / self.entry_price
            r = int(gp / 0.10) * 0.2
            em = max(1.5, self.atr_stop_mult - r)
            self.stop_price = max(self.stop_price, self.highest_since_entry - em * atr_val)
        if side == 1 and self.entry_price > 0:
        ...
        if side == 1 and not np.isnan(self._gbt[i]) and self._gbt[i]<0.4:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and not np.isnan(self._gbt[i]) and self._gbt[i]>0.6:
