...
class StrategyV28(TimeSeriesStrategy):
    """
    策略简介：Adaptive Kalman趋势 + Stochastic回调入场的做多策略（10min频率）。

    使用指标：
    - Adaptive Kalman(60): 自适应Kalman滤波器趋势线
    - Stochastic(14,3): 超卖回调入场
    - ATR(14): 止损距离计算

    进场条件（做多）：价格 > Adaptive Kalman 且 %K < 30（回调）
    出场条件：ATR追踪止损 / 分层止盈 / 价格跌破Kalman线

    优点：自适应Kalman比固定参数更灵活，Stochastic精确入场
    缺点：Kalman在跳变行情中可能过度平滑
    """
    name = "mt_v28"
    warmup = 1000
    freq = "10min"
    ...
    def on_init_arrays(self, context, bars):
        ...
        sk, _ = stochastic(highs, lows, closes, k=self.stoch_k, d=self.stoch_d)
        self._stoch_k = sk
    ...
    def on_bar(self, context):
        ...
        kf = self._kalman[i]
        sk = self._stoch_k[i]
        if np.isnan(atr_val) or np.isnan(kf) or np.isnan(sk):
            return

        self.bars_since_last_scale += 1
        above_kalman = price > kf
        ...
        if side == 0 and above_kalman and sk < self.stoch_entry:
