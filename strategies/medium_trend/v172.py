...
class StrategyV172(TimeSeriesStrategy):
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._knn, _ = knn_signal(closes, features, period=120, k=5)
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not np.isnan(self._knn[i]) and self._knn[i]<0:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and not np.isnan(self._knn[i]) and self._knn[i]>0 and not np.isnan(self._er[i]) and self._er[i]>0.4:
