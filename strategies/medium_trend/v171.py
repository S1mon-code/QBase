...
class StrategyV171(TimeSeriesStrategy):
    """
    策略简介：Isolation异常过滤+Supertrend做多（5min）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._anom = isolation_anomaly(features, period=120)
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not np.isnan(self._st_dir[i]) and self._st_dir[i]==-1:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and not np.isnan(self._st_dir[i]) and self._st_dir[i]==1 and i>0 and self._st_dir[i-1]==-1 and not np.isnan(self._anom[i]) and self._anom[i]>-0.5:
