...
class MediumTrendV240(TimeSeriesStrategy):
    """
    策略简介：Reflex + TrendFlex双Ehlers振荡器的1h做多策略。

    使用指标：
    - Reflex(20): Ehlers反射指标，趋势快速反应
    - TrendFlex(20): Ehlers趋势弹性指标，趋势延续确认
    - ATR(14): 止损距离计算
    ...
    - ATR追踪止损（3.5倍ATR）
    - 分层止盈
    - Reflex < 0 且 TrendFlex < 0（双指标同时转弱）

    优点：Ehlers双指标互补，数字信号处理基础扎实
    缺点：参数敏感，非主流指标经验较少
    """
    name = "mt_v240"
    warmup = 60
    ...
    def on_bar(self, context):
        ...
        tf_val = self._tf[i]
        if np.isnan(ref_val) or np.isnan(tf_val):
            return
        ...
        if side == 1 and ref_val < 0 and tf_val < 0:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and ref_val > 0 and tf_val > 0:
