...
class StrategyV157(TimeSeriesStrategy):
    """
    策略简介：日线Kalman + 4h Supertrend + 5min Volume Spike三周期策略。

    使用指标：
    - Kalman Filter [日线]: 自适应趋势上升
    - Supertrend(10,3.0) [4h]: 方向确认
    - Volume Spike(20,2.0) [5min]: 量突增+阳线入场
    - ATR(14) [5min]: 止损距离

    进场条件（做多）：日线Kalman上升 + 4h ST=1 + 5min量突增阳线
    出场条件：ATR追踪止损, 分层止盈, Kalman或ST翻转

    优点：Kalman自适应去噪+ST方向+量价确认
    缺点：Volume Spike信号稀少
    """
    name = "medium_trend_v157"
    freq = "5min"
    warmup = 3000
    ...
    def on_bar(self, context):
        ...
        if np.isnan(atr_val) or np.isnan(k_cur) or np.isnan(k_prev) or np.isnan(sd):
            return
        ...
        if side == 1 and (not kalman_up or not daily_up):
            context.close_long()
            self._reset_state()
            return

        if side == 0 and kalman_up and daily_up and is_spike and is_green:
