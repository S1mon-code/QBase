...
class StrategyV186(TimeSeriesStrategy):
    """
    策略简介：Break-even逐级提升+结构突破入场做多（4h）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_init_arrays(self, context, bars):
        ...
        _,_,self._bs = cusum_break(closes, period=60, threshold=3.0)
    ...
    def on_bar(self, context):
        ...
        if side == 1 and self.entry_price > 0:
            ...
            nl = max(0, int(pa) - 1)
            bl = getattr(self, "be_level", 0)
            if nl > bl:
                self.be_level = nl
                self.stop_price = max(self.stop_price, self.entry_price + self.be_level * atr_val)
        if side == 1 and self.entry_price > 0:
        ...
        if side == 0 and not np.isnan(self._bs[i]) and self._bs[i]==1:
