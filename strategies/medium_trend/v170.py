...
class StrategyV170(TimeSeriesStrategy):
    ...
    def on_bar(self, context):
        ...
        if side == 1 and not np.isnan(self._ts[i]) and self._ts[i]<30:
            context.close_long()
            self._reset_state()
            return

        if side == 0 and not np.isnan(self._ts[i]) and self._ts[i]>60 and not np.isnan(self._cmf[i]) and self._cmf[i]>0.05:
