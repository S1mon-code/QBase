...
class StrategyV181(TimeSeriesStrategy):
    """
    策略简介：Chandelier Exit+ADX趋势入场做多（日线）。
    进场条件（做多）：核心信号满足阈值
    ...
    def on_init_arrays(self, context, bars):
        ...
        self._ch, _ = chandelier_exit(highs,lows,closes,period=22,multiplier=3.0)
    ...
    def on_bar(self, context):
        ...
        if side == 0 and not np.isnan(self._adx[i]) and self._adx[i]>25 and not np.isnan(self._ch[i]) and price>self._ch[i]:
