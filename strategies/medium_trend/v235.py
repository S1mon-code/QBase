...
class MediumTrendV235(TimeSeriesStrategy):
    """
    策略简介：CCI超卖回升 + ZLEMA零滞后均线方向的1h做多策略。

    使用指标：
    - CCI(20): 商品通道指数，-100~+100正常区间
    - ZLEMA(20): 零延迟指数均线
    - ATR(14): 止损距离计算

    进场条件（做多）：
    - CCI从 < -100回升至 > -100（超卖回升）
    - 价格 > ZLEMA（趋势向上）

    出场条件：
    - ATR追踪止损（3.0倍ATR）
    - 分层止盈
    - CCI > 200后回落至 < 100（超买衰竭）

    优点：CCI超卖入场胜率高，ZLEMA零滞后跟踪快
    缺点：CCI在强趋势中可能长期维持超买
    """
    name = "mt_v235"
    warmup = 60
    ...
    def on_bar(self, context):
        ...
        cci_val = self._cci[i]
        zl = self._zlema[i]
        if np.isnan(cci_val) or np.isnan(zl):
            return
        ...
        if side == 1:
            if cci_val > 200:
                self.cci_was_high = True
            if self.cci_was_high and cci_val < 100:
                context.close_long()
                self._reset_state()
                return

        if side == 0 and prev_cci < -100 and cci_val > -100 and price > zl:
