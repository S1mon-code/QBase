...
class StrategyV86(TimeSeriesStrategy):
    """
    策略简介：EMA(20/50)金叉 + OBV趋势确认的4h多头策略。

    使用指标：
    - EMA Cross(20, 50): 快慢均线交叉判断趋势方向
    - OBV: 量价趋势确认，OBV上升确认资金流入
    - ATR(14): 止损距离计算

    进场条件（做多）：
    - EMA20 > EMA50（金叉后）
    - OBV在上升（当前OBV > 20bar前OBV）
    ...
    缺点：均线交叉滞后，震荡市频繁假信号
    """
    name = "medium_trend_v86"
    warmup = 200
    freq = "4h"

    ema_fast: int = 20       # Optuna: 10-30
    ema_slow: int = 50       # Optuna: 30-80
