...
class StrategyV13(TimeSeriesStrategy):
    """
    策略简介：Aroon趋势方向 + CMO动量确认的做多策略（5min频率）。

    使用指标：
    - Aroon(25): Aroon Up > 70 且 Aroon Down < 30 确认上升趋势
    - CMO(14): Chande动量振荡器 > 0 确认动量方向
    - ATR(14): 止损距离计算

    进场条件（做多）：Aroon Up > 70 且 Aroon Down < 30 且 CMO > 0
    出场条件：ATR追踪止损 / 分层止盈 / Aroon Down > 70

    优点：Aroon基于高低点位置，不依赖价格平滑
    缺点：Aroon在窄幅震荡时可能两个都低
    """
    name = "mt_v13"
    warmup = 2000
    freq = "5min"
    ...
    def on_bar(self, context):
        ...
        cmo_val = self._cmo[i]
        if np.isnan(atr_val) or np.isnan(a_up) or np.isnan(a_down) or np.isnan(cmo_val):
            return

        self.bars_since_last_scale += 1
        bullish = a_up > self.aroon_up_threshold and a_down < self.aroon_down_threshold and cmo_val > 0
        ...
        if side == 1 and a_down > self.aroon_up_threshold:
            context.close_long()
            self._reset_state()
            return
