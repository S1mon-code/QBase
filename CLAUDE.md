...
目标函数覆盖三个维度：收益、风险控制、利润质量。

**多品种（Strong Trend，跨 6 个训练品种）：**
```python
score = mean_sharpe + 0.3 * min_sharpe - dd_penalty - concentration_penalty
         ─────────   ──────────────────   ──────────   ─────────────────────
          基础收益      一致性奖励           风险控制        利润质量
```

**单品种（All-Time AG，只跑 AG 训练集）：**
```python
score = sharpe - dd_penalty - concentration_penalty
```
...
`0.3 * min_sharpe` — 奖励在**所有品种**上都盈利的参数组合。如果 6 个品种中 5 个 Sharpe=2.0 但 1 个 Sharpe=-1.0，这一项会惩罚它（-0.3），逼迫优化器找到跨品种一致的参数。

#### 组件 2: 回撤惩罚（非线性阶梯式）

小回撤不罚，大回撤加速惩罚，匹配实盘风险容忍度：

| MaxDD 区间 | 每超 1% 的惩罚 | 累计惩罚示例 |
|-----------|:---:|------|
| ≤ 15% | 0 | dd=15% → 0 |
| 15% - 25% | 0.3 | dd=20% → 0.015, dd=25% → 0.030 |
| 25% - 35% | 0.8 | dd=30% → 0.070, dd=35% → 0.110 |
| > 35% | 2.0 | dd=40% → 0.210, dd=50% → 0.410 |

**效果：** Sharpe=1.5 / MaxDD=-40% 的参数得分 1.29（vs 无惩罚的 1.50），40% 回撤在实盘中是灾难，优化器会主动避开它。

#### 组件 3: 利润集中度惩罚

防止"3 笔交易撑起整个 Sharpe"的幸存者偏差。衡量指标：**top 10% 的天数贡献了多少比例的总利润**（从回测权益曲线计算）。

| 集中度 | 含义 | 惩罚 |
|-------|------|------|
| ≤ 0.5 | 利润分布健康 | 0 |
| 0.5 - 0.7 | 有些集中，但可接受 | 轻罚（0.3 per 0.1） |
| > 0.7 | 依赖少数暴利天，脆弱 | 重罚（0.6 per 0.1） |

**参考值：** 正态收益策略 ≈ 0.3-0.5，趋势策略 ≈ 0.4-0.5，暴利依赖策略 ≈ 0.8-0.95。

**优雅降级：** 仅当 AlphaForge 返回权益曲线时生效，否则跳过（不影响已有回测）。

#### 组件 4: 交易次数门槛（硬过滤）

低于门槛的回测结果直接被排除（返回 -10.0），确保参数在统计上有意义：
