...
QBase 的策略开发遵循 **5.5 步流水线**，每一步有明确的输入、输出和标准：
...
| 第四五步：归因分析 | 验证结果 | 归因报告 | 信号ablation + 行情regime |
...
使用 `tanh` 曲线评分，自然递减回报，无硬性 benchmark：
...
| 1.5 | 7.8 | 优秀 |
| 2.0 | 8.9 | 卓越 |
| 3.0 | 9.7 | 接近满分，继续提升意义不大 |

**为什么用 tanh：** Sharpe 从 0.5 到 1.0 的提升（+2.7 分）比 2.0 到 2.5 的提升（+0.6 分）更有价值。这防止优化器为了微小的 Sharpe 提升而牺牲其他维度。

**多品种一致性处理：** 多品种评估时，如果最差品种 Sharpe 为负，effective_sharpe 会被缩减（乘以一致性系数 0.5-1.0），然后再用 tanh 评分。
...
| 频率 | 最低交易次数 | 说明 |
|------|:---:|------|
| daily | 10 | 趋势策略月均 1-2 笔，10 笔 ≈ 半年 |
| 4h | 20 | 每周 2-3 笔，20 笔 ≈ 2 个月 |
